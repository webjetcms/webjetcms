...
let pageIdText = "128791";
let pageIdImage = "128791";
let pageIdCK = "14";
let pageIdPageBuilder = "123";
...
Scenario('ai-assistants usage - IMAGE - WITH prompt', async ({ I, DTE, Document }) => {
    ...
    I.fillField("#ai-user-prompt", "Izometrický pohľad na bratislavký hrad.");
    I.click( locate("#toast-container-ai-content").find(locate("button").withText("Generovať")) );
...
Scenario('ai-assistants usage - TEXT - PageBuilder', async ({ I, DTE, Document }) => {
    ...
    await I.executeScript(() => {
        $("#DTE_Field_data-pageBuilderIframe").contents().find("span#cke_73").css({"outline": "1px solid #FF4B58", "box-shadow": "0 0 12px #FF4B58"})
    });

    Document.screenshot("/redactor/ai/datatables/page_builder.png");

    await I.executeScript(() => {
        $("#DTE_Field_data-pageBuilderIframe").contents().find("span#cke_73").css({"outline": "none", "box-shadow": "none"})
    });
...
Scenario('ai-assistants usage - page imgaes', async ({ I, DTE, Document }) => {
