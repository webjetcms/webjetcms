...
export function update(EDITOR, action) {
    ...
    $.each(fields, function(i, v) {
        ...
        if (v.type == 'textarea') {
            ...
            template = '<div class="custom-field-ai-disabled"> ' + template + ' <div class="vueComponent" id="DTE_Field_' + customPrefix + identifier + '"><webjet-dte-jstree :data-table-name="dataTableName" :data-table="dataTable" :click="click" :id-key="idKey" :data="data" :attr="attr"></webjet-dte-jstree></div> </div>';
        } else if (v.type == 'json_group' || v.type == 'json_doc') {
            template = '<div class="custom-field-tree custom-field-ai-disabled"> ' + template + ' <div class="vueComponent" id="DTE_Field_' + customPrefix + identifier + '"><webjet-dte-jstree :data-table-name="dataTableName" :data-table="dataTable" :click="click" :id-key="idKey" :data="data" :attr="attr"></webjet-dte-jstree></div> </div>';
        } else if (v.type == 'none') {
            // LPA
            container.hide();
