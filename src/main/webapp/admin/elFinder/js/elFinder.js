	...
	*/
	...
	$.each(this.commands, function(name, cmd) {
		...
		if ($.isFunction(cmd) && !self._commands[name] && (cmd.prototype.forceLoad || $.inArray(name, self.options.commands) !== -1)) {
			...
			if (typeof self._commands[name].linkedCmds != "undefined" && self._commands[name].linkedCmds.length) {
							...
							if (data.error) {
								...
								if (fm.options.overwriteUploadConfirm && ! fm.UA.iOS && fm.option('uploadOverwrite', target)) {
									if (data.list) {
										...
										if (existed.length > 0 && $.isPlainObject(existed[0])) {
											existed = $.map(existed, function(v, i){
												return v.name;
											});
										}
										...
										{
											//console.log("exists, test="+name.name.toLowerCase());
											var existsLC = $.inArray(name.name.toLowerCase(), existed) !== -1 ? name : null ;
											var exists = $.inArray(name.name, existed) !== -1 ? name : null ;
                                            return existsLC || exists;
										});
