...
-webkit-box-shadow: 3px 3px 1px 0px rgba(50, 50, 50, 0.16);
-moz-box-shadow:   3px 3px 1px 0px rgba(50, 50, 50, 0.16);
box-shadow:         3px 3px 1px 0px rgba(50, 50, 50, 0.16);
}

#testImonials<%=testImonialsId %> .testimonials-text p{
padding:0px !important;
}
#testImonials<%=testImonialsId %> .testimonials-arrow{
    ...
    display: block;
    color:<%=pageParams.getValue("backgroundColor", "#fff")%>;
    height: 20px;
    position: absolute;
    top: 23px;
    left:-11px;
...
<%
String orientationAvatar = "left";
String orientationText ="right";
String orientationInnerText = "left";

String arrow = "testimonials-arrow";
%>
<% for(int i = 0; i < itemsList.length(); i++) { %>
	<% JSONObject item = itemsList.getJSONObject(i); %>	
				...
				%>
				<div class="testimonials_content <%=easyingStyle %>" style="<%=style%>">
				
				<div class="testimonials-text" style="float:<%=orientationText %>; text-align:<%=orientationInnerText %>;">
				<span class="inner">
			  ...
			  <span class="testimonials-avatar" style="float:<%=orientationAvatar %>; <% if(item.get("image")!=null){ %>background-image:url(/thumb<%=item.get("image")%>?w=100&h=100&ip=5);"<%} %>></span>
			  <%} 
				if(orientationAvatar.equals("left")){
					orientationAvatar = "left; margin-left:0px;";
					orientationText = "left; margin-left:50px; -webkit-box-shadow: -3px 3px 1px 0px rgba(50, 50, 50, 0.16); -moz-box-shadow:-3px 3px 1px 0px rgba(50, 50, 50, 0.16); box-shadow:-3px 3px 1px 0px rgba(50, 50, 50, 0.16);";
					orientationInnerText = "right";
					arrow = "testimonials-arrow2";
						
					}else{
						orientationAvatar = "left";
						orientationText = "right";
						orientationInnerText = "left";
						arrow = "testimonials-arrow";
					}
			  %>
