...
<%@page import="sk.iway.iwcm.system.monitoring.ExecutionTimeMonitor"%>
<%@page import="sk.iway.iwcm.system.monitoring.MonitoringDB"%>
<%@ include file="/admin/layout_top.jsp" %>
<%@page import="sk.iway.iwcm.database.SimpleQuery"%>
<%@page import="sk.iway.iwcm.system.monitoring.ExecutionEntry"%>
<c:catch var="exc">
...
<div class="row title">
    <h1 class="page-title"><i class="fa icon-equalizer"></i><iwcm:text key="components.monitor.monitoring"/><i class="fa fa-angle-right"></i><iwcm:text key="components.monitor.components_monitoring"/></h1>
</div>
...
<%
	...
	else
	{
		...
		List<ExecutionEntry> stats = ExecutionTimeMonitor.statsForComponents();
		...
		if (requestsStatsOnAnotherNode)
			stats = MonitoringDB.getComponentStatsFor(Tools.getRequestParameter(request, "node"));
			...
			<form action="<%=PathFilter.getOrigPath(request)%>" method="get">
				...
				<div class="col-lg-12">
					<div class="form-group">
						<iwcm:text key="components.monitoring.nodeName"/>:
				...
				<div class="col-lg-3 col-sm-6">
					<div class="form-group">
						<label class="form-label" for="nameFilter"><iwcm:text key="components.monitor.component_name"/>:</label>
...
<c:if test="<%=requestsStatsOnAnotherNode%>">
	<% Date when = new SimpleQuery().forDate(
		...
		Constants.getString("clusterMyNodeName"), MonitoringDB.TYPE_COMPONENT);  %>
	<p>
...
<display:table uid="stat" name="stats" export="true" pagesize="20" requestURI="<%=PathFilter.getOrigPath(request)%>" excludedParams="docid" class="sort_table">
	...
	<display:column titleKey="components.monitoring.component_name" sortable="true" property="name" style="text-align: left;" />
	...
	<display:column titleKey="components.monitoring.component_cache_hits" sortable="true" property="numberOfCacheHits" decorator="sk.iway.displaytag.NumbersDecorator" />
	<display:column titleKey="components.monitoring.component_cache_execution_time" sortable="true" property="averageCacheExecutionTime" decorator="sk.iway.displaytag.NumbersDecorator" />
	...
	<display:column titleKey="components.monitoring.average_memory_consumed" sortable="true" property="averageMemoryConsumption" decorator="sk.iway.displaytag.NumbersDecorator" />
	<display:column titleKey="components.monitoring.consumption_peek" sortable="true" property="memoryConsumptionPeek" decorator="sk.iway.displaytag.NumbersDecorator" />
</display:table>
</logic:notEmpty>
