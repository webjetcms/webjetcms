...
<div class="row title">
    <h1 class="page-title"><i class="fa icon-equalizer"></i><iwcm:text key="components.monitor.monitoring"/><i class="fa fa-angle-right"></i><iwcm:text key="components.monitor.documents_monitoring"/></h1>
</div>
...
else
{
	...
	List<ExecutionEntry> stats = ExecutionTimeMonitor.statsForDocuments();
	...
	if (requestsStatsOnAnotherNode)
		stats = MonitoringDB.getDocumentStatsFor(Tools.getRequestParameter(request, "node"));
	    ...
	    <div id="tabMenu1" class="tab-pane active">
   			...
   			<form action="<%=PathFilter.getOrigPath(request)%>" method="get">
				...
				<div class="col-lg-3 col-sm-6">
					<div class="form-group">
						<label class="form-label" for="nameFilter"><iwcm:text key="components.monitor.documents_url"/>:</label>
...
<c:if test="<%=requestsStatsOnAnotherNode%>">
	<% Date when = new SimpleQuery().forDate(
		...
		Constants.getString("clusterMyNodeName"), MonitoringDB.TYPE_DOCUMENT);  %>
	<p>
...
<display:table uid="stat" name="stats" export="true" pagesize="20" requestURI="<%=PathFilter.getOrigPath(request)%>" excludedParams="docid" class="sort_table">
	...
	<display:column titleKey="URL" sortable="true" property="name" style="text-align: left;" />
