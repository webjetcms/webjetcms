        ...
        $.fn.gridEditor = function( options ) {
            ...
            self.each(function(baseIndex, baseElem) {
                ...
                var settings = $.extend({
                    ...
                    <%-- TODO: takto sa vola novy servis pre ulozenie elementu. Vypis takto ulozenych elementov zatial nieje. --%>
                    <%--,{--%>
                        <%--title: '<iwcm:text key="grideditor.section.save" />',--%>
                        <%--iconClass: 'fa-floppy-o',--%>
                        <%--on: { click: function() {--%>
                            <%--if(confirm('Naozaj chcete ulozit tento blok?')){--%>
                                <%--$.post("/admin/grideditor/save/element", {html : "<h1>Nejaky vzorovy html kod</h1>", name: "Náš obľúbený blok č.5" }, function(data, textStatus) {--%>
                                    <%--&lt;%&ndash;createSideBar(data, groupId);&ndash;%&gt;--%>
                ...
                function getJsonObjectSelectGroup(){
                    <%--var jsonObjectSelectGroup = JSON.parse(JSON.stringify(<%@include file="grideditor_add_section_source_JSONARRAY_ajax.json"%>));--%>
                    jsonObjectSelectGroup = JSON.parse('[]');
                    $.getJSON( "/admin/grideditor/categories", function(data) {
                ...
                function bindFunctionSelectGroupGroup(category_groups){
                    category_groups.find('.wj-ge-group').not('.wj-ge-more-group').on('click',function(){
                        ...
                        if( !$this.is('[data-action]') ) {
                            ...
                            $.post("/admin/grideditor/category", {categoryDirId : categoryId }, function(data, textStatus) {
                ...
                function showSelectGroup($this){
                    ...
                    <%--$this.parent().addClass('isOpened').find('i').removeClass('fa-plus').addClass('fa-minus');--%>
                ...
                function hideSelectGroup(detach){
                    ...
                    <%--canvas.find('.ge-tools-drawerPlusSection').removeClass('isOpened').find('i').removeClass('fa-minus').addClass('fa-plus');--%>
                ...
                function showHideSelectGroup($this) {
                    ...
                    <%--var isFirstPlusSection = $this.parent().hasClass('ge-tools-dPS-canvas');--%>
                ...
                function createSideBar(jsonObjectSideBar, clickedGroupId){
                    ...
                    <%--var jsonObjectSideBar = JSON.parse(JSON.stringify(<%@include file="grideditor_sidebar_source_ajax.json"%>));--%>
