		...
		<iwcm:notPresent name='<%=Constants.USER_KEY+"_changepassword"%>'>
			...
			<form action="/usrlogon.do" method="post" name="logonForm" class="form-horizontal">
				...
				<div class="col-sm-12 text-center">
							...
							<input type="submit" class="btn btn-success login-submit" value="<iwcm:text key="button.submit"/>" />
							<% if (submitURL != null)
							{
			...
			<p class="text-center">
				<a href="#sendPassword" onclick="$('#sendPassword').show('slow'); return false"><iwcm:text key="logon.didYouForgetPassword"/></a>
			</p>
			<div id="sendPassword" style="display: none;">
				<fieldset>
					<br/><p><iwcm:text key="logon.lost_password_message"/></p>
					<form id="sendPasswd" name="f_passwd" method="post" action="<%=PathFilter.getOrigPath(request)%>" class="form-horizontal">
						<div class="form-group">
							<label class="col-md-2 control-label"><iwcm:text key="user.email"/></label>
							<div class="col-md-10">
