...
`regexes-webjet.yaml` is synchronized from `regexes.yaml` in the Apache-2.0 licensed
...
The updater validates the downloaded file before replacing `regexes-webjet.yaml` and records the resolved upstream commit in `upstream-commit.txt`. Review and commit both files together.
