...
public abstract class ExpressionExecutorSupport implements ExpressionExecutor {
  ...
  /**
   * ELResolver that maps top-level variables to ActionBean properties and special self/this
   * keywords to currently evaluated value.
   */
  protected static class BeanVariableELResolver extends ELResolver {
    private final BeanVariableResolver variableResolver;

    BeanVariableELResolver(BeanVariableResolver variableResolver) {
    ...
    public Object getValue(ELContext context, Object base, Object property) {
      if (base == null && property != null) {
        ...
        return variableResolver.resolveVariable(property.toString());
      }
      return null;
    }
    ...
    public void setValue(ELContext context, Object base, Object property, Object value) {
      // Validation expressions are read-only in this context.
    }
  ...
  protected static class ValidationELContext extends ELContext {
    private final CompositeELResolver resolver;

    ValidationELContext(BeanVariableResolver variableResolver) {
      this.resolver = new CompositeELResolver();
      this.resolver.add(new BeanVariableELResolver(variableResolver));
  ...
  public void evaluate(
    ...
    BeanVariableResolver resolver = new BeanVariableResolver(bean);
    ELContext context = new ValidationELContext(resolver);
    ValueExpression expr = null;

    if (validationInfo.expression() != null) {
      try {
        ExpressionFactory expressionFactory = getExpressionFactory();
        if (expressionFactory == null) return;

        String expression = toValueExpression(validationInfo.expression());
        expr = expressionFactory.createValueExpression(context, expression, Boolean.class);
    ...
    for (Object value : values) {
      ...
      if (expr != null) {
        try {
          ...
          Boolean result = (Boolean) expr.getValue(context);
   ...
   *
   * @return a working ExpressionFactory implementation.
   */
  protected abstract ExpressionFactory getExpressionFactory();

  private String toValueExpression(String expression) {
