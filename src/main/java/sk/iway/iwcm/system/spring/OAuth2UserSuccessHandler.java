...
public class OAuth2UserSuccessHandler implements AuthenticationSuccessHandler {
    ...
    public void onAuthenticationSuccess(HttpServletRequest request, HttpServletResponse response, Authentication authentication) throws IOException {
        try {
            ...
            // Aplikuj user groups z OAuth2 iba pre Keycloak
            if (isKeycloakProvider(oauth2User)) {
                applyOAuth2UserGroups(oauth2User, userDetails);
            } else {
                Logger.info(OAuth2UserSuccessHandler.class, "Skipping group synchronization for non-Keycloak provider for user: " + userDetails.getEmail());
            }
    ...
    private boolean isKeycloakProvider(OAuth2User oauth2User) {
        ...
        Logger.debug(OAuth2UserSuccessHandler.class, "Checking if provider is Keycloak - hasKeycloakAttributes: " + hasKeycloakAttributes);
        return hasKeycloakAttributes;
    }

    /**
     * Aplikuje user groups z OAuth2 atribútov na používateľa (iba pre Keycloak)
