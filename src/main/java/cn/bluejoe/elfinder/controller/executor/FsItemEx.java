...
public class FsItemEx
{
	...
	public List<FsItemEx> listChildren(int selectedType) {
		...
		if(selectedType == sk.iway.iwcm.system.elfinder.FsService.TYPE_IMAGES) {
			...
			for (FsItem child : childrens) {
				String mimeType = _v.getMimeType(child);
				if("directory".equals(mimeType) || mimeType.startsWith("video/") || mimeType.startsWith("audio/") || FileTools.isVideoFile(_v.getName(child)) || FileTools.isAudioFile(_v.getName(child))) {
					list.add(new FsItemEx(child, _s));
				}
			}
			...
			for (FsItem child : childrens) {
				...
				if("directory".equals(mimeType) || mimeType.startsWith("image/") || mimeType.startsWith("video/") || mimeType.startsWith("audio/") || FileTools.isImage(_v.getName(child)) || FileTools.isVideoFile(_v.getName(child)) || FileTools.isAudioFile(_v.getName(child))) {
					list.add(new FsItemEx(child, _s));
				}
			}
		} else {
