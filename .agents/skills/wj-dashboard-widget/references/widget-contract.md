...
- Optional `headerLink: {href, labelKey?}` for a safe local module link. Without `labelKey` the title becomes a link; with it a separate translated link sits beside the title. Header navigation survives loading, errors, collapse and title updates. Use it instead of a duplicate footer link.
