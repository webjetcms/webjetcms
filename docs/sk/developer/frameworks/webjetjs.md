...
- ```closeButtonPosition``` = pozícia tlačidla na zatvorenie okna
  - ```prázdna hodnota``` - malá ikona X v hlavičke okna
...
- ```buttons``` = pole vlastných tlačidiel v pätičke dialógu, každé tlačidlo je objekt s vlastnosťami:
  - ```title``` (String) - text tlačidla
  - ```cssClass``` (String) - CSS trieda tlačidla (napr. `btn-primary`, `btn-outline-secondary`)
  - ```onClick``` (funkcia) - callback volaný po kliknutí na tlačidlo

  Ak parameter `buttons` nie je zadaný, zobrazí sa predvolené tlačidlo Potvrdiť (správanie sa nezmení). Príklad:
...
Dialóg podporuje **presúvanie okna** uchopením za hlavičku (drag & drop) — kurzor sa zmení na `grab`. Na zariadeniach s malou výškou obrazovky (pod 760px) alebo v `iframe` režime sa okno zobrazí maximalizované a presúvanie nie je dostupné.

V hlavičke okna sú dostupné tlačidlá pre **maximalizáciu a minimalizáciu** (prepnutie do celo obrazovkového režimu). Na zariadeniach s malou výškou obrazovky (pod 760px) sú tieto tlačidlá skryté a okno sa zobrazí automaticky maximalizované.
