...
- CKEditor - doplnená možnosť konfigurovať pravidlá čistenia obsahu pri vložení z Word/Excel cez premennú `ckeditor_pasteFromWord_disallowedContent` vrátane zložených ACF pravidiel. **Upozornenie:** predvolené čistenie po novom odstraňuje aj CSS triedy a atribúty `align` a `valign` z hlavičkových buniek `TH`, ide o zámernú zmenu oproti predchádzajúcej verzii (#300).
