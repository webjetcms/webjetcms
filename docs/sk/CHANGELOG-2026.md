...
- CKEditor - doplnená možnosť [konfigurovať ACF pravidlá čistenia obsahu](frontend/setup/ckeditor.md#čistenie-html-kódu-pri-vložení-z-wordexcel) pri vložení z Word/Excel cez premennú `ckeditor_pasteFromWord_disallowedContent` v štandardnom editore aj v režime Page Builder. Podporované sú bodkočiarkou oddelené zložené pravidlá; každé sa spracuje samostatne, takže neplatné pravidlo neblokuje ostatné. Prázdna hodnota vypne toto dodatočné filtrovanie a hodnota sa do JavaScriptu prenáša bezpečnou JSON serializáciou. **Upozornenie:** v štandardnom editore predvolené čistenie po novom odstraňuje aj atribút `nowrap` z buniek `TD` a CSS triedy s atribútmi `align` a `valign` z buniek `TH`. Page Builder namiesto pôvodného užšieho filtra používa rovnaký predvolený zoznam, preto navyše odstraňuje CSS triedy a atribúty `align`, `valign`, `nowrap` z `TD`, CSS triedy a atribúty `align`, `valign` z `TH`, atribút `align` z `P` a atribút `width` z `COL`. Ide o zámerné zmeny oproti predchádzajúcej verzii (#300).
