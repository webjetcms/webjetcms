...
Atribútom `basePath` nastavíte koreňový priečinok pre jednotlivé súbory, aby ste nemuseli písať celú cestu. Pre adresy začínajúce na `/templates/` alebo `/components/` sa hodnota v `basePath` nepridá.
