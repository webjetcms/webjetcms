...
Cez Page Builder sa zobrazujú aj bloky ako hlavička, pätička, menu atď., čo v niektorých prípadoch nemusí byť žiadúce. Jednoduché riešenie je tieto bloky cez CSS štýl chovať. V režime editora je na `body` elemente CSS trieda `is-edit-mode` ktorá vám umožní schovať nepotrebné elementy.
