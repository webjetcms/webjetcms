...
Pri potvrdení sa vybraná kombinácia uloží do URL obrázka aj CSS triedy `fixedSize`. Ak obsahuje kvalitu, trieda má navyše príponu `-q90` (napríklad `fixedSize-730-401-5-ff00ff-true-q90`). Pri opätovnom otvorení sa zvolí zodpovedajúca povolená možnosť. Ak pôvodný rozmer nie je povolený, pred potvrdením treba vybrať platnú možnosť. V ostatných režimoch zostávajú samostatné polia dostupné.
