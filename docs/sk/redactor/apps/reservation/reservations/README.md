...
Aplikácia "Rezervácie" obsahuje uzol "Zoznam rezervácií", ktorý umožňuje vytváranie/úpravu/mazanie rezervácií, ako aj ich import z Excel súboru a export do Excel/CSV súboru (prípadne aj možnosť okamžitého tlačenia pri exporte). Zoznam obsahuje aj 3 tlačidlá na schvaľovanie/zamietnutie rezervácie, viď sekcia [Schvaľovanie rezervácií](#schvaľovanie-rezervácií).
...
Zmena stavu rezervácie je možná buď pomocou editoru a to presnejšie v karte "Schvaľovanie", ktorá sa zobrazuje iba za určitých špecifických podmienok, alebo pomocou tlačidiel na zmenu stavu rezervácie.
...
![](approve_sure.png)  ![](reject_sure.png)  ![](reset_sure.png)
...
Mazanie rezervácií je závislé od toho, nad akým rezervačným objektom bola táto rezervácia vytvorená. Rezervačný objekt môže mať nastavené heslo, ktoré zmení priebeh vymazávania rezervácie - heslo sa nastavovalo v karte [Pokročilé](../reservation-objects/README.md).
...
-V prípade, že vymazávate viac rezervácií nad tým istým objektom (ktorý má heslo), tak budete vyzvaný iba jeden krát k zadaniu tohoto hesla (nemusíte ho duplicitne zadávať pre každú rezerváciu zvlášť).
