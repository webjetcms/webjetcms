...
!> Warning: version **2026.0** is the latest version for the Tomcat 9 application server using `javax namespace`. All newer versions from `2026.18` will be determined **just for** application server **Tomcat 10/11** using `Jakarta namespace`. Change from `javax` at `jakarta namespace` is a consequence of the transfer of `Java EE` specifications from Oracle to **Open Source/Eclipse Foundation** where the project continues under the name `Jakarta EE`. This change requires an update of the application server, as Tomcat 10 and later versions are already using exclusively `jakarta namespace` for all `Java EE` API. For a current list of available versions, see [installation section](install/versions.md).
