...
In [cluster](README.md#cluster) mode, you can increase security for a publicly accessible node by setting a database user with read-only permissions on selected (sensitive) tables. In addition to correctly setting database table permissions, set the following configuration variables for this node:

- `clusterMyNodeType` - ​​set to `public` to make this node run without administration and restrict some table writes.
- `monitoringEnableCountUsersOnAllNodes` - ​​set to `false` to prevent writing to the configuration table. More in [server monitoring](../../sysadmin/monitoring/README.md).

## Table rights

Example of setting permissions for user `WEB_WJ_APP` with read-only permissions for an Oracle database. User `WEB_WJ` is a full-privileged user and the owner of the schema with the data.
...
Example for MariaDB, schema name is `webjetcms_web`, user with read rights is `webjetcms_web_ro`:
...
For the Oracle database, it is necessary to create `trigger` to change the schema after login and set up a procedure for creating statistical tables.
