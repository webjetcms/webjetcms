...
The values ​​in `minBrowserVersion` can be supplemented with new browsers by entering the value of the detected browser from the value in `${ninja.userAgent.browserName}`.

For example, for `User-Agent` the value:

```
Mozilla/4.0 (compatible; MSIE 7.0; Windows NT 5.1; Trident/4.0; GTB6.6; InfoPath.1; Crazy Browser 3.0.3)
```

you can enter a check for minimum version 10 in `config.properties`:

```properties
minBrowserVersion.crazy\u0020browser=10
```

Note the special character `\u0020` which represents a space. The value is entered in lowercase.

## Text version of the page *Boolean*

If the parameter `forceBrowserDetector` is set to `blind`, it returns `true`.

```java
${ninja.userAgent.blind}
```

Example of enabling in URL address:
```url
www.nazovdomeny.sk?forceBrowserDetector=blind
```

Example of disabling in URL address:
```url
...
Device type (always lowercase) detected from userAgent:
...
The name of the operating system (always lowercase) on the device as determined from the UserAgent, e.g. `windows 7, macos high sierra`.
...
## Example of browser detection on the backend (in a JSP file)
