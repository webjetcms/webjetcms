...
```html
Web stránka: <iwcm:write name="field_a" /> až <iwcm:write name="field_t" />
...
ATTENTION: **type setting** must always be set for the default language (set in the configuration variable `defaultLanguage`), even if WebJET is used in another language.
...
- `editor.field_x.type=text` - standard text field
