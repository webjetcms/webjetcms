...
The minimum PostgreSQL 16 is based on the [WebJET CMS core requirements](../../../../install/setup/README.md#core-requirements-for-server). **pgvector 0.5.0** is the functional minimum for the HNSW index that WebJET creates; the extension must also support the chosen PostgreSQL version. **pgvector 0.8.1 or later** is required for **PostgreSQL 18**, but we recommend current patches. The pgvector 0.8 series also brought improvements to HNSW performance and query planning with filters. For details, see the [pgvector changelog](https://github.com/pgvector/pgvector/blob/master/CHANGELOG.md).

For MariaDB, the versions listed refer to **Community Server** used in the development Docker. Native vectors were added in 11.7, but the minimum for WebJET is **11.8**, the first LTS line with this feature. All vector features used by WebJET are available in 11.8; upgrading to 12.x is not a requirement for semantic search. See [MariaDB Vector Overview](https://mariadb.com/docs/server/reference/sql-structure/vectors/vector-overview).

When choosing a MariaDB version, consider:

- **11.8 LTS** - a foundation for broader client compatibility. Use current fixes: version [11.8.3](https://mariadb.com/docs/release-notes/community-server/11.8/11.8.3) fixed vector index corruption when rolling back a statement in a transaction, and [11.8.9](https://mariadb.com/docs/release-notes/community-server/11.8/11.8.9) fixed a decrease in the success rate of finding the closest results when loading the cosine index from disk.
...
| Variable | Default value | Description |
| --- | --- | --- |
| `ragSearchEfSearch` | `40` | HNSW parameter `ef_search`. A higher value improves recall, but may slow down searches. MariaDB allows values ​​from `1` to `10000` ; if invalid, its database default value remains unchanged. |
