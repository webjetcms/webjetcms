...
**Doplnění dokumentace se dobře provádí** během code review v **merge requeste**. Proto si vždy připravte ```merge request``` a následně v listu ```Changes``` projděte všechny změněné soubory. **Při každé změně se zamyslete**, zda je z kódu srozumitelná a zda nemodifikuje aktuální chování, nebo nedoplňuje vlastnosti stávajícího kódu. Všímejte si:
...
Doporučuji vám mít **otevřené okno vyhledávání** v ```docs``` složce, a když vidíte v ```Changes``` listu změnu v nějaké proměnné/konfiguraci/objektu, tak ji vyhledáte v dokumentaci (složku ./docs). Pokud ji naleznete, text doplníte o změnu, pokud nenajdete, doplníte celý popis změny.
