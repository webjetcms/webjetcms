...
Přístup k historii je **pouze pro čtení**, v REST službě je zápis zakázán v metodě ```beforeDelete``` a ```beforeSave```.
...
- při kliknutí na tlačítko Přidat nebo Upravit složku nad stromovou strukturou se zapamatuje jméno tlačítka v proměnné ```groupsDatatableClickButtonAfterXhr``` a vyvolá se tlačítko ```buttons-refresh``` v datatabulce pro načtení dat ze serveru
- po načtení dat ze serveru se ověří stav v proměnné ```groupsDatatableClickButtonAfterXhr``` a není-li ```null```, tak se vyvolá zadané tlačítko v datatabulce (což otevře editor adresáře).
...
Vstupní pole pro zadání ```docid``` ve stránce je implementováno přímo ve [web-pages-list.pug](../../../../src/main/webapp/admin/v9/views/pages/webpages/web-pages-list.pug) z předpraveného HTML kódu ```#docIdInputWrapper```, který je přestěhován do kont. Po stisknutí klávesy ```Enter``` je vyvolána akce otevření zadaného ID stránky.
...
Klepnutí na list načte obsah datatabulky jakoby pro adresář nastavený v konfigurační proměnné ```systemPagesRecentPages```, který se kontroluje ve třídě [WebpagesService](../../../../src/main/java/sk/iway/iwcm/editor/rest/WebpagesService.java) se a.

## Zobrazení karty Čekající na schválení

Zobrazení seznamu v kartě čekající na schválení je komplikovanější. Na backendu se kontroluje, zda pro aktuálně přihlášeného uživatel existují stránky, které má schválit. Pokud ano, nastaví se ve třídě [WebPagesListener](../../../../src/main/java/sk/iway/iwcm/editor/rest/WebPagesListener.java) do modelu atribut ```hasPagesToApprove``` na hodnotu ```TRUE```. Tento se testuje ve [web-pages-list.pug](../../../../src/main/webapp/admin/v9/views/pages/webpages/web-pages-list.pug) a pokud je ```FALSE``` tak se list skryje.

Po kliknutí na kartu se do datatabulky načte jakoby adresář s ID nastaveným v konfigurační proměnné ```systemPagesDocsToApprove```, což se kontroluje ve třídě [WebpagesService](../../../../src/main/java/sk/iway/iwcm/editor/rest/WebpagesService.java). Načtení komplikuje fakt, že jako unikátní ID se používá hodnota ```docId```. Pro schvalování ale může existovat více záznamů pro stejné ```docId```. Jako nejjednodušší řešení jsme otočili hodnotu ```docId``` a ```historyId```, které se navzájem v přenášených datech vymění.

Problémem je zavolání obnovení dat po kliknutí, které volá ```WebpagesRestController.getOne```, kde je zaslána hodnota ```historyId``` jako ```id```. Podle zaslaného ```id``` (což je ve skutečnosti ```historyId```) získáme dotazem do databáze korektní hodnotu ```docId``` pro získání obsahu podle kombinace ```docId, historyId```.

Klepnutí je obsloužené ve funkci ```webpagesDatatable.onEdit``` otevřením nového okna ```/admin/approve.jsp``` se schvalovacím dialogem.
...
U web stránek ale nastal problém v tom, že samotná editace probíhá v ```iframe``` elementu. Událost tedy nebyla korektně nastavena. Událost je třeba přidat jak pro ckeditor, tak pro PageBuilder manuálně a vyvolat událost v rodičovském rámci.
