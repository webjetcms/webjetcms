...
Hodnota v `click` se nastaví přímo na `onclick` atributu tlačítka, nesmí obsahovat znak ", doporučujeme jen volat vhodnou JS funkci.

## Potvrzení akce

Pro potvrzení akce (ekvivalent ```window.confirm``` kde kliknutím na OK/Potvrdit mohu provést zvolenou akci) je připravena JS funkce ```WJ.confirm(options)```. V ```options``` objektu mohou být následující parametry:
...
- ```WJ.formatTimeSeconds(timestamp)``` - ​​naformátuje zadaný ```timestamp``` jako čas včetně sekund
...
Pomocí volání ```WJ.openIframeModal(options)``` lze otevřít dialogové okno s iframe zadané URL adresy. Neotevírá se tak ```popup``` okno, ale dialogové okno přímo ve stránce. V ```options``` objektu mohou být následující parametry:
...
- ```closeButtonPosition``` = pozice tlačítka pro zavření okna
  - ```prázdna hodnota``` - ​​malá ikona X v hlavičce okna
  - ```close-button-over``` - ​​ikona X v hlavičce ale přes obsah okna (nevytváří samostatný řádek)
  - přidáním ```nopadding```, tzn. ```closeButtonPosition: "close-button-over nopadding"``` se zruší i horní odsazení v hlavičce
- ```okclick``` = callback po kliknutí na tlačítko potvrdit, neobsahuje žádné parametry, hodnotu z iframe je třeba si vytáhnout v implementaci callback-u
- ```onload``` = callback po načtení okna, jako parametr dostane ```event.detail``` obsahující objekt ```window``` s odkazem na okno v iframu
- ```buttons``` = pole vlastních tlačítek v patičce dialogu, každé tlačítko je objekt s vlastnostmi:
  - ```title``` (String) - text tlačítka
  - ```cssClass``` (String) - CSS třída tlačítka (např. `btn-primary`, `btn-outline-secondary`)
  - ```onClick``` (funkce) - callback volaný po kliknutí na tlačítko

  Pokud parametr `buttons` není zadán, zobrazí se výchozí tlačítko Potvrdit (chování se nezmění). Příklad:
...
Dialog podporuje **přesouvání okna** uchopením za hlavičku (drag & drop) — kurzor se změní na `grab`. Na zařízeních s malou výškou obrazovky (pod 760px) nebo v `iframe` režimu se okno zobrazí maximalizované a přesouvání není dostupné.

V hlavičce okna jsou dostupná tlačítka pro **maximalizaci a minimalizaci** (přepnutí do celo obrazovkového režimu). Na zařízeních s malou výškou obrazovky (pod 760px) jsou tato tlačítka skrytá a okno se zobrazí automaticky maximalizovaná.
...
HTML kód dialogu je staticky vložen v souboru [iframe.pug](../../../../src/main/webapp/admin/v9/views/modals/iframe.pug) a linkováno do stránky v [layout.pug](../../../../src/main/webapp/admin/ Iframe se tedy opakovaně používá pro různé dialogy. V proměnné ```modalIframe``` je odkaz na instanci dialogu.
...
## Modální okno s aktuálním kontextem

Pro otevření modálního okna s aktuálním kontextem stránky (CSS styly, JavaScript soubory a objekty) lze použít volání `WJ.openModal`. Načtená URL adresa nebude vložena do `iframe` elementu ale bude provedena vrámci aktuálně načtené web stránky. Příklad použití:

```pugjs
a.dropdown-item.passkey(onclick="WJ.openModal({url: '/admin/v9/users/passkey/', title: this.textContent.trim()})")
```

Samotný načtený HTML soubor nesmí obsahovat vloženou `hlavičku/menu`, aby nevznikla duplicita JavaScript/HTML/CSS objektů. Příkladem souboru je [passkey.pug](../../../../src/main/webapp/admin/v9/views/pages/users/passkey.pug).
...
Ochrana pro CSRF tokeny a spojení se serverem je kromě časovače nastavena také v [head.pug](../../../../src/main/webapp/admin/v9/views/partials/head.pug) v nastavení ajax volání pomocí funkce ```$.ajaxSetup```. Pro HTTP chybu se stavem 401 je volána funkce ```WJ.keepSessionShowLogoffMessage()```, pro chybu 403 funkce ```WJ.keepSessionShowTokenMessage(errorMessage)```.
...
- ```tabs``` - ​​pole zobrazených položek navigační lišty
  ...
  - ```title``` - ​​název položky
  - ```active``` - ​​(volitelné) pokud je false zobrazí se jako neaktivní možnost - používá se pro pod-stránky aplikace kde první položka odkazuje na úvodní/hlavní stránku aplikace
- ```backlink``` - ​​(volitelné) odkaz na předchozí stránku (používá se v ```master-detail``` zobrazení, např. v detailu formuláře odkaz na seznam formulářů)
- `showInIframe` - ​​(volitelné) pokud je nastaveno na `true`, nebo v kartách existuje atribut s hodnotou `title: '{filter}'` zobrazí se hlavička iv `iframe` elementu - typicky ve vlastnostech aplikace v editoru stránek
...
Pokud v navigační liště potřebujete mít externí filtr, lze použít jako titulek značku ```{TEXT}```. Pokud titulek začíná na znak ```{``` vloží se text obalený do DIV kontejneru. To je následně možné použít pro přesun [externího filtru](../datatables/README.md#externí-filtr) jako je nap. v aplikaci GDPR/Vyhledávání.
...
Při zobrazení webové stránky je vygenerován objekt ```window.nopermsJavascript``` se seznamem práv, která uživatel nemá povoleno. Toto pole nikdy nepoužívejte přímo, pro kontrolu práv použijte API volání:
...
Pokud potřebujete ukládat některá nastavení uživatele můžete použít ```window.localStorage``` objekt. Ten ale zůstane uložen jen v prohlížeči. Pokud potřebujete nastavení pro uživatele mít stejná ve všech prohlížečích, nebo je mít dostupná i na serveru je třeba využít možností ```UserDetails.adminSettings```, které se ukládají do databázové tabulky ```user_settings_admin```. Uloženy jsou ve formátu klíč/hodnota, kde hodnota často bývá JSON objekt.
...
Na backendu je možné použít třídu ```AdminSettingsService``` pro získání údajů:
...
- `WJ.debugTimer(message)` - ​​vypíše hlášení s časovým údajem od první zprávy. Hlášení je třeba zapnout voláním `WJ.debugTimer(true)`, jinak se nezobrazí. Není tam potřeba zakomentovat všechna hlášení v kódu.