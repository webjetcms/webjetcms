...
[Thymeleaf](https://www.thymeleaf.org/doc/tutorials/3.0/usingthymeleaf.html#standard-expression-syntax) je šablonovací framework. Ve WebJETu je spojen se Spring MVC.
...
Text nelze vypsat přímo, je nutné jej dát do např. ```span``` obalovače s atributem ```data-th-text```, což nahradí obsah ```span``` elementu (aby se dalo prototypovat).
Podobně se nastavují i ​​atributy, tzn. ```data-th-href=...```
...
- ```int getConstantInt(String name)``` - ​​vrátí celočíslní hodnotu konstanty podle zadaného ```name```, pokud neexistuje vrátí 0.
- ```int getConstantInt(String name, int defaultValue)``` - ​​vrátí celočíslní hodnotu konstanty podle zadaného ```name```, pokud neexistuje vrátí hodnotu zadanou v ```defaultValue```.
...
Pokud potřebujete volat komplexnější kód můžete jej vložit do statické metody a tu následně zavolat v PUG souboru:
...
av PUG souboru ji zavoláte následovně:
