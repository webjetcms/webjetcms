...
Datum poslední změny je zároveň v [head.pug](../../../../src/main/webapp/admin/v9/views/partials/head.pug) dostupné jako ```window.propertiesLastModified```. Ušetří se tím jedno volání REST služby, pokud klíče není třeba aktualizovat.
...
U nových klíčů doporučujeme používat příponu ```.js``` ve jménu klíče pro jeho automatické zařazení do odpovědi REST služby (např. ```datatables.error.system.js```).
