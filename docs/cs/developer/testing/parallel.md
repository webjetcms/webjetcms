...
Z tohoto důvodu je připraven skript ```singlethread```, který spustí pouze testy označené značkou ```@singlethread``` v názvu scénáře. Zároveň takto označené testy jsou vyloučeny z paralelního spuštění.
