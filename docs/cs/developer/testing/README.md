...
Příklad komplexnějšího testu pro otestování přihlášení [src/test/webapp/tests/admin/login.js](../../../../src/test/webapp/tests/admin/login.js):
...
Pomocí zápisu ```within``` můžete omezit element, na který se použijí následující příkazy:
...
- [await I.clickIfVisible(selector)](../../../../src/test/webapp/custom_helper.js) - pokud je daný element zobrazen klikne na něj, pokud zobrazený není přeskočí krok (nevyhodí chybu)
...
Pro datatabulku máme připraveny speciální funkce. Jsou implementovány v [DT.js](../../../../src/test/webapp/pages/DT.js):
...
- ```DT.filter(name, value, type=null)``` - ​​nastaví hodnotu ```value``` do textového filtru sloupce ```name``` datatabulky. Pokud je zadán i atribut ```type```, nastaví se typ hledání (např. Začíná na, Končí na, Rovná se).
...
- ```DTE.waitForEditor(name)``` - ​​čeká na zobrazení editoru, pokud je definován název, použije se datatabulka s daným jménem, ​​ve výchozím nastavení ```datatableInit```
...
- ```DTE.save(name)``` - ​​klikne na tlačítko Uložit v editoru, je-li definován název, použije se datatabulka s daným jménem, ​​ve výchozím nastavení ```datatableInit```
- ```DTE.cancel(name)``` - ​​klikne na tlačítko zavření editoru, je-li definován název, použije se datatabulka s daným jménem, ​​ve výchozím nastavení ```datatableInit```
- ```DTE.fillField(name, value)``` - ​​vyplní standardní pole, na rozdíl od volání ```I.fillField``` lze do ```name``` parametru zadat přímo jméno pole na backendu/json definici.
...
- ```DTE.fillCleditor(parentSelector, value)``` - ​​zadá text do WYSIWYG ```cleditor```. Hodnota ```parentSelector``` - odkaz na element ve kterém se ```cleditor``` nachází (např. ```#forum```), ```value``` - hodnota k vyplnění. **Upozornění:** neví to zatím diakritiku z důvodu použití ```type``` příkazu.
Pro datatabulku lze provést i [automatizovaný test](datatable.md).
...
- ```I.jstreeNavigate(pathArray)``` - ​​v poli ```pathArray``` lze definovat jména jednotlivých uzlů ve stromové struktuře, na které funkce postupně klikne např. ```I.jstreeNavigate( [ "English", "Contact" ] );```.
...
Funkce implementované v ```Document``` objektu:
...
- `notifyClose` - ​​zavře `toastr` oznámení.
- `notifyCheckAndClose(text)` - ​​ověří text v `toastr` notifikaci a zavře ji.
...
Obecně se nedoporučuje používat ```I.wait``` s fixní dobou. Čas potřebný pro čekání může být odlišný na lokálním počítači a v CI/CD pipeline. Navíc fixní doba může zbytečně prodlužovat dobu potřebnou k provedení testu.
...
V souboru [codecept.conf.js](../../../../src/test/webapp/codecept.conf.js) je definováno také přihlašování přes rozšíření [autologin](https://codecept.io/plugins/#autologin):
...
Pro vytvoření univerzálních testovacích scénářů je složka `Pages` do které se generují Page objekty
přes příkaz `npx codeceptjs gpo`, vytvoří se page objekt pomocí ```Dependency Injection``` (podobně jako v Angular).
...
Voláním adresy stránky s parametrem ```removePerm``` lze za běhu odebrat zadané právo přihlášenému uživateli (bez uložení změn v právech), pokud přihlašovací jméno uživatele začíná na ```tester```. Je tak možné testovat zobrazení stránky bez zadaného práva a ověřit tak bezpečnost volání REST služeb.

Odebrání práva je implementováno ve funkci ```DT.checkPerms(perms, url)``` v [DT.js](../../../../src/test/webapp/pages/DT.js). Vyžaduje zadat právo a adresu stránky na které se právo testuje. Testo ověřuje zobrazení oznámení ```Prístup k tejto stránke je zamietnutý```.
 Volitelný parametr ```datatableId``` reprezentuje ID/jméno tabulky ve stránce (je třeba zadat pokud je ve stránce více datatabulek).
...
Do parametru ```removePerm``` lze zadat i více práv oddělených čárkou.

U datatabulek je možné nastavovat i práva na [jednotlivá tlačítka](../datatables/README.md#tlačítka-dle-práv) (přidat, editovat, duplikovat, smazat). Testovat tak můžete i jednotlivě vypnutá práva. Pro ověření práv na backendu je ale třeba testovat i REST službu. Přidáním výrazu ```forceShowButton``` do parametru ```removePerm``` u uživatele s přihlašovacím jménem začínajícím na ```tester``` se tlačítka v datatabulce zobrazí. Je tak možné otestovat zobrazení chybového hlášení z REST služby (že skutečně záznam nelze přidat/editovat/smazat). Příklad je ve ```webpage-perms.js```:
...
- ```selector``` - ​​```selector``` elementu, ze kterého se má pořídit snímek (nedělá se z celé obrazovky, ale pouze ze zadaného elementu)
