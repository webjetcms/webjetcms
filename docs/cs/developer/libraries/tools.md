...
| [System](#system)                     | [String](#string)                         | [Dům](#dům)                                       | [Test](#test)                     | [Other](#other)       |
| -----------                           | -----------                               | -----------                                       | -----------                       | -----------           |
...
Ověří, zda se nacházíme v developerském prostředí.
Pokud ano, funkce vrátí **TRUE** ne-li tak vrátí **FALSE**.
Hodnota je zároveň zpřístupněna i ve `window` pod klíčem `isDev` (`window.isDev`).
```javascript
/**
...
Vyparsuje z adresy GET query za otazníkem a vrátí objekt s hodnotami `{kľúč : hodnota}`.
Ošetřuje také query umístěnou za #hash atributem.
```javascript
/**
...
Aktualizuje url search query vstupními hodnotami.
Pokud se v poli `excludeKeys` nacházejí názvy klíčů, nebudou v aktualizaci přidány.
Nastavení prázdné hodnoty pomocí ```queryValue``` způsobí odstranění parametru z URL adresy.
```javascript
/**
...
Nahradí ze vstupního textu všechny znaky s diakritikou za znaky anglické abecedy.
Zároveň máme k dispozici volitelný argument vložení znaku nebo řetězce,
kterým budou ve vstupním textu nahrazeny všechny mezery.
```javascript
/**
...
Pokud potřebujeme pomocí Javascriptu nastavit nějakému elementu úplně nejvyšší dostupný `z-index`,
tak si tento nový nejvyšší můžeme zjistit zavoláním dané funkce.
```javascript
/**
...
Otestuje zadanou vstupní hodnotu, zda je prázdná.
Je možné vložit i více vstupních hodnot současně oddělených čárkou a je-li alespoň jedna prázdná, tak metoda vrátí TRUE.
```javascript
/**
...
Zkontroluje, zda existuje element na základě jeho css selektoru.
Pokud neexistuje, tak metoda poslouchá av případě jeho vytvoření zavolá `existCallback`.
Na element se čeká maximálně 25 sekund. Potom se poslech přeruší a zavolá se `notExistCallback`
```javascript
/**
