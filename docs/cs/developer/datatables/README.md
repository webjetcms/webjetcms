...
- ```tabs {json}``` definice [karet pro editor](../datatables-editor/README.md#karty-v-editore).
...
- ```editorId {string}``` jednoznačný identifikátor editoru, pokud není zadaný použije se hodnota ```id```. Potřebné hlavně pokud máte v jedné web stránce více datatabulky.
...
- ```fetchOnCreate {boolean}``` po nastavení na true bude před vytvořením nového záznamu provedeno REST volání s hodnotou -1 pro získání dat nového objektu. Hodnoty se nastaví voláním ```EDITOR.setJson(json)``` implementované v ```$.fn.dataTable.Editor.prototype.setJson``` v eventu ```initCreate```.
...
- ```hideButtons {string}``` čárkou oddělený seznam jmen tlačítek, která se mají automaticky v datatabulce schovat (nezobrazit), například. ```create,edit,duplicate,remove,import,celledit```.
...
- ```perms``` nastaví [práva pro zobrazení tlačítek](#tlačítka-dle-práv) pro přidání, editaci, duplikování a mazání údajů
- ```lastExportColumnName``` pokud je zadané zobrazí v exportním dialogu možnost exportovat dosud neexportované údaje (používá se ve formulářích). Hodnota reprezentuje jméno sloupce, který se přidává jako ```NULL``` podmínka do výběru dat (je třeba korektně implementovat v REST službě).
...
- ```editorButtons``` pole tlačítek, které se zobrazí v editoru. Příklad ```editorButtons: [ {title: "Uložiť", action: function() { this.submit(); } }, { title: ...} ]```. Využívá API pro Datatables Editor.
...
- ```stateSave {boolean}``` - ​​nastavením na hodnotu ```false``` deaktivujete možnost pamatování si pořadí sloupců a uspořádání tabulky v prohlížeči.
- ```customFieldsUpdateColumns {boolean}``` - ​​nastavením na hodnotu ```true``` se při získání [volitelných polí](../datatables-editor/customfields.md) aktualizují i ​​názvy sloupců v tabulce a v nastavení zobrazených sloupců (ve výchozím nastavení při hodnotě ```false``` se názvy volitelných pole) aktualizují pouze v editoru.
...
- ```editorLocking {boolean}``` - ​​ve výchozím nastavení tabulka volá službu oznámení při editaci stejného záznamu více uživateli, pokud je toto nechtěné nastavte na hodnotu `false`.
...
- `toggleSelector` – CSS selektor určující, který element spouští výběr řádku v tabulce. Výchozí hodnota je `td.dt-select-td`, což znamená, že výběr řádku se provede pouze kliknutím na buňku s touto třídou (typicky sloupec s ID). Toto nastavení můžete přepsat (např. na hodnotu `tr`), aby bylo možné vybrat řádek kliknutím kdekoli na řádek.
- `toggleStyle` – režim výběru řádků. Výchozí hodnota je `multi` (lze vybrat více řádků najednou). Nastavením na hodnotu `single` omezíte výběr pouze na jeden řádek (vhodné, pokud má být najednou editovatelný pouze jeden řádek).
...
Přes API lze v ```toolbare``` odebrat/přidat tlačítka:
...
Pokud potřebujete zobrazovat tlačítka podle práv (např. tlačítko Přidat pouze pokud má uživatel určité právo) je možné do konfigurace datatabulky přidat atribut ```perms```:
...
!>**Upozornění:** nespoléhejte se pouze na kontrolu práv na frontendu, práva je třeba kontrolovat iv REST službě nebo v service třídě. Využít můžete metody [beforeSave nebo beforeDelete](restcontroller.md#zabránění-smazání--editace-záznamu).

## Stylování řádku

Někdy je třeba nastavit CSS styl celého řádku (např. tučné písmo pro hlavní stránku, nebo červené pro nedostupnou). K přenosu těchto doplňkových údajů využíváme přenos vnořených atributů přes objekt [EditorFields](../datatables-editor/datatable-columns.md#vnořené-atributy). Vytvořili jsme třídu [BaseEditorFields](../../../../src/main/java/sk/iway/iwcm/system/datatable/BaseEditorFields.java), která má metodu ```addRowClass(String addClass)``` pro přidání CSS třídy na řádku.
...
Nastavení CSS stylu řádku je implementováno v [index.js](../../../../src/main/webapp/admin/v9/npm_packages/webjetdatatables/index.js) pomocí možnosti ```rowCallback``` konstruktoru datatabulky. Ověřuje existenci property ```data.editorFields.rowClass``` a existuje-li tak hodnotu aplikuje na řádek.

Nastavení stylu řádku můžete provést také v JavaScript kódu (např. na základě atributů) pomocí volby ```onRowCallback```. Můžete tak snadno označit řádky jako neaktivní CSS stylem ```is-not-public```.
...
Někdy je třeba zobrazit stavové ikony záznamu (např. ve web stránkách ikony Nezobrazené v menu, Přesměrovaná stránka a podobně). K přenosu těchto doplňkových údajů využíváme přenos vnořených atributů přes objekt [EditorFields](../datatables-editor/datatable-columns.md#vnořené-atributy). Vytvořili jsme třídu [BaseEditorFields](../../../../src/main/java/sk/iway/iwcm/system/datatable/BaseEditorFields.java), která má metodu ```addStatusIcon(String className)```. Ikony jsou ze sady FontAwesome.
...
WebJET při zobrazení stránky vygeneruje do HTML kódu JS pole ```nopermsJavascript``` ve kterém je seznam modulů, na které uživatel nemá práva. Rovněž se vygeneruje i CSS styl se třídami ```.noperms-menomodulu``` s nastaveným ```display: none```.

## Uspořádání

Datatable podporuje nastavení uspořádání atributem [order:](https://datatables.net/reference/option/order). Tento lze přenášet jako ```option``` při inicializaci tabulky. Z důvodu pugjs/thymeleaf parsingu ale nelze zapsat přímo výraz ```[[0, 'asc']]```, protože jej Thymeleaf provede. Je třeba pole uspořádání připravit oklikou přes proměnnou a push:
...
Chcete-li přesunout filtr do záhlaví stránky, můžete jej jednoduše přesunout pomocí jQuery jako je v [database-delete.pug](../../../../src/main/webapp/admin/v9/views/pages/settings/database-delete.pug).
...
Pokud potřebujete dynamicky měnit možnosti výběrového pole `select` je nutné kromě změny `option` objektů nastavit i atribut `_editor_val`, který se použije jako zvolená hodnota. Příklad je pro vnořenou datatabulku, kde bylo třeba na základě hodnoty načíst možnosti do výběrového pole.
...
- `mode`, povinný parametr, určuje jakým způsobem se budou data sloupce sčítat. Možné hodnoty:
  - `all`, sečtou se všechny hodnoty sloupce (ze všech stran), čili změna strany v tabulce hodnotu nezmění
...
Pokud je tabulka nastavena jako `serverSide: false`, neboli data se nestránkují, při počítání hodnot se neprovede žádný `request` na databázi, protože tabulka má již všechna potřebná data v sobě.

Pokud je tabulka nastavena jako `serverSide: true`, tedy data se stránkují, akce se mění podle zvoleného módu:
...
Jelikož `footer` využívá data tabulky (až na jeden případ), výsledná hodnota sloupce závisí na vy-filtrovaných datech. Takto umíte snadno zjistit celkovou hodnotu sloupců pro specifické parametry.

!>**Upozornění:** Pokud je tabulka nastavena jako `serverSide: true` a mód patičky je `all`, sečtené hodnoty se **nemění** v závislosti na filtrování v tabulce.
