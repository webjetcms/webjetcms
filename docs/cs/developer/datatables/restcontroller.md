...
Java třída [DatatableRestControllerV2](../../../../src/main/java/sk/iway/iwcm/system/datatable/DatatableRestControllerV2.java) zapouzdřuje komunikaci s [datatabulkou](README.md) a [editorem](../datatables
...
!>**Upozornění:** v datové entitě **NESMÍTE používat primitivní typy** jako ```int```, ```long``` ale objekty ```Integer```, ```Long```, jinak nebude fungovat vyhledávání. To používá ExampleMatcher, který NULL objekty nevloží do DB dotazu. Pro primitivní typy ale nemůže použít NULL, nastaví je na hodnotu 0 a následně přidá do ```WHERE``` podmínky.
...
Pokud Spring DATA repozitář implementuje rozhraní `DomainIdRepository`, třída `DatatableRestControllerV2` pracuje s hodnotou `domainId`. Při čtení se automatické filtrování aktivuje pro MultiWeb/Cloud instalaci nebo při zapnuté konfigurační proměnné `enableStaticFilesExternalDir`. Při zápisu se pro takový repozitář vždy doplní chybějící `domainId` a ověří se, že zaslaná hodnota odpovídá očekávané doméně.

Očekávanou doménu vrací chráněná metoda `getDomainId()`. Její výchozí implementace používá aktuální doménu z `CloudToolsForCore.getDomainId()`. Hodnota se používá při standardním získání jednoho nebo všech záznamů, mazání, vyhledání záznamu přes `findItemBy`, výpočtu částek a při doplnění nebo kontrole `domainId` během uložení.

Pokud má controller pracovat s jinou než aktuální doménou, přepište metodu `getDomainId()`. Stejnou metodu použijte i ve vlastních repozitářových dotazech controlleru:
...
V příkladu controller spravuje pomocné záznamy `CustomFieldsEntity`, ale jejich doménový rozsah určuje `EnumerationDataBean`. Resolveru se proto předává anotovaná kontextová třída, ne nutně typ entity použité v repozitáři.

Anotace [`@DomainIdCommon`](../../../../src/main/java/sk/iway/iwcm/system/multidomain/DomainIdCommon.java) označuje entitu, jejíž data jsou společná pro všechny domény. Je dostupná během běhu aplikace a vztahuje se i na podtřídy. Vyhodnocuje ji třída [`DomainIdScopeResolver`](../../../../src/main/java/sk/iway/iwcm/system/multidomain/DomainIdScopeResolver.java):

- pro anotovanou třídu vrátí hodnotu konfigurační proměnné `domainIdCommon` ; pokud není nastavena na kladné číslo, použije doménu `1`,
- pro neanotovanou nebo neznámou třídu vrátí aktuální doménu,
- třídu lze zadat jako `Class<?>` nebo její úplné jméno (`FQDN`).

Entitu označíte následovně:
...
Samotná anotace nemění databázové dotazy automaticky. Controller musí použít `DomainIdScopeResolver` v přepsané metodě `getDomainId()` a služby s vlastními dotazy musí použít výsledek resolveru explicitně. Označení proto používejte pouze pro data, která mají být skutečně sdílena a upravována ze všech domén.

## Metody pro manipulaci s daty

Třídu můžete konstruovat is repozitářem ```NULL```, v takovém případě je třeba implementovat metody pro práci s daty:
...
Pokud potřebujete upravit data před uložením, nebo provést akci po uložení do databáze (např. nastavit datum uložení, nebo ```domainId```) použijte metodu ```beforeSave``` nebo ```afterSave```:
...
Voláním metody ```setForceReload(true);``` lze vynutit obnovení dat datatabulky po uložení.
...
Pro select boxy v editoru a automatickou konverzi ID na hodnota (např. ```templateId``` na jméno šablony) je ve WebJETu přidáno rozšíření ```PageImpl``` objektu [DatatablePageImpl.java](../../../../src/main/java/sk/iway/iwcm/system posílat číselníková data.
...
Při volání ```page.addOptions``` nastavuje třetí ```true/false``` parametr přidání původního ```Java Beanu``` do výstupního objektu. Je-li nastaveno na ```true```, v JSON objektu bude původní Bean dostupný v objektu original. Lze toho využít k získání dat např. šablony:
...
Kromě obecné metody ```addSpecSearch``` (která interně volá následující speciální metody) je možné použít metody ze třídy [SpecSearch](../../../javadoc/cs/iway/iwcm/system/datatable/SpecSearch.html):
...
Pokud máte zobrazenou cestu k objektu ```DocDetails``` (typicky u pole typu json) je možné k filtrování podle cesty použít metodu ```addSpecSearchDocFullPath(String paramValue, String jpaProperty, List<Predicate> predicates, Root<T> root, CriteriaBuilder builder)```:
...
Do editoru je integrována standardní Java validace [jakarta.validation](https://www.baeldung.com/javax-validation). Je tedy třeba v ```JPA Beane``` nastavit požadované validační anotace:
...
Při použití ```@Convert``` anotace s využitím objektu místo primitivní hodnoty (např. ```GroupDetailsConverter```) je na entitě třeba použít anotaci ```@Valid```. Ta následně vyvolá validaci vnořeného objektu. Technický problém je, že chyba se vyvolá v ```group.navbarName```, ale takové pole neexistuje v datatabulce. Třída ```DatatableExceptionHandlerV2``` toto ošetřuje tak, že pro pole obsahující znak ```.``` v názvu do vystupného JSON objektu generuje jako jméno pole hodnotu jen před znakem ```.```, čili např. . ```group```. Výjimka je pole s názvem ```editorFields```, které se ponechá v původním/plném tvaru.
...
Pokud potřebujete speciálně kontrolovat práva (např. u webových stránek povolení ke složkám) můžete implementovat metodu `public boolean checkItemPerms(T entity, Long id)`. Metoda se standardně jmenuje při operacích editace/vytvoření/smazání/provedení akce/získání záznamu:
...
Pro import je možné modifikovat/validovat data implementací metody `preImportDataEdit`. Tato metoda je volána před samotným importem a lze v ní data modifikovat. Příklad je ve třídě [EnumerationDataRestController](../../../../src/main/java/sk/iway/iwcm/components/enumerations/rest/EnumerationDataRestController.java).
