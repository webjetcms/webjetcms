...
Implementace je v souboru ```export-import.js``` ve funkci ```bindExportButton(TABLE, DATA) ```, která je inicializována přímo v ```index.js``` při inicializaci datatabulky. HTML kód exportního dialogu je v souboru ```datatables-data-export.pug``` který je přes ```include``` vkládán do ```layout.pug```.

### Příprava dat před exportem

V datatabulce se často načítají jen základní data/sloupce (hlavně při použití [editorFields](../datatables-editor/customfields.md) atributu). Proto je před exportem vždy třeba volat server a získat kompletní data. Při detekci exportu se místo ```ProcessItemAction.GETALL``` použije ```ProcessItemAction.GETONE``` jako při získání jednoho záznamu. Vrácené entity tedy budou obsahovat všechna data.
...
v atributu ```data-hide``` lze specifikovat seznam elementů, které se automaticky schovají po nastavení uvedené možnosti.

Je třeba implementovat JS funkci ```window.exportDialogCustomCallback(type, TABLE)```, která je provedena u této možnosti:
...
> **Upozornění:** importovat je technicky možné i jen některé sloupce, nepředpokládejte tedy, že import bude vždy obsahovat všechna data. Jinak vám budou nastávat chyby typu ```NullPointerException```. Zvláště v ```editorFields.toEntity``` je třeba kontrolovat ```null``` hodnoty na atributech, aby jejich přenos nepadal.
...
- ```@DataTableColumn(className = "not-export")``` - ​​sloupec s CSS třídou ```not-export``` nebude exportován.
...
Je třeba implementovat JS funkci ```window.importDialogCustomCallback(type, TABLE)```, která je provedena u této možnosti:
...
Třída ```DatatableRestControllerV2``` obsahuje metody ```isExporting()``` pro detekci exportu a ```isImporting()``` importu. Tyto lze využít v implementaci vašeho REST controlleru, např. pro validaci dat v metodě ```validateEditor```.