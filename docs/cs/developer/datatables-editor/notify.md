...
konstruktor [NotifyBean](../../../javadoc/cs/iway/iwcm/system/datatable/NotifyBean.html) třídy má následující parametry:
...
Do notifikace je možné přidat tlačítko pro provedení akce (např. načtení poslední stránky z historie). Pomocí API metody [NotifyBean.addButton](../../../javadoc/cs/iway/iwcm/system/datatable/NotifyBean.html) můžete přidat tlačítko (objekt typu [NotifyButton](../../../javadoc/cs/iway/iwcm/system Po klepnutí na tlačítko se provede zadaná JavaScript funkce (vkládá se jako atribut ```onlick``` na tlačítku). Definování volané funkce musíte implementovat přímo v dané stránce.
...
Notifikace jsou během běhu REST služby ukládány do ```ThreadLocal``` objektu ```ThreadBean```, aby je bylo možné přidat kdykoliv. Následně jsou generovány do výstupního JSON objektu:
...
- ```refreshRow(id, callback)``` - ​​toto volání je použito při ```fetchOnCreate/fetchOnEdit```, notifikace se získají z ```json.editorFields.notify``` objektu

Samotné zobrazení notifikace je implementováno ve funkci ```showNotify(notifyList)``` která pro každý prvek seznamu notifikace volá zobrazení přes [WJ.notify](../frameworks/webjetjs.md#notifikace)
