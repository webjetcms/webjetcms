...
Kde ```sk.iway.iwcm.components.gallery.GalleryEntity``` je objekt řízen přes DataTables.
...
Příklad vlastní [render](https://datatables.net/reference/option/columns.render) funkce:
...
- ```title``` - ​​není-li zadán automaticky se vygeneruje jako ```components.meno_beanu_bez_dto_alebo_bean_na_konci.name``` - https://datatables.net/reference/option/columns.title, viz dokumentace k [překladům](#překlady-názvů-sloupců). **Upozornění:** pokud je title prázdný (nebo tvrdá mezera) tak se automaticky sloupci nastaví atribut ```hidden=true```.

Volitelná pole:

- ```tab``` - ​​zkratka pro nastavení ```{editor: {tab: String}}```
- ```className``` - ​​doplňkový CSS styl nastavený na ```TD``` elementu, chcete-li nahradit CSS styl nastavený pomocí ```inputType``` zadejte jej se znakem ```!``` na začátku - https://datatables.net/reference/option/columns.className. Existují speciální CSS třídy:
  ...
  - `hide-on-create` - ​​schová pole při vytváření nového záznamu.
...
- `renderFunction` - ​​jméno funkce v JavaScript, která se použije pro vlastní vykreslení hodnoty sloupce. Více na [stránce DataTables](https://datatables.net/reference/option/columns.render).
- `renderFormatAriaLabel` - ​​hodnota atributu `aria-label` pro vygenerovaný odkaz přes `renderFormat`, třeba použít pokud se zobrazuje pouze ikona a ne text.
- `sortAfter` - ​​jméno pole za které se přidá toto pole v pořadí
- ```editor``` - ​​objekt ```DataTableColumnEditor```
- ```hidden``` - ​​pole se nezobrazí v datatabulce a uživatel si ho na rozdíl od ```visible``` nemůže zobrazit, pole může být použito v editoru
...
- `alwaysCopyProperties` - ​​při editaci záznamu se u `null` vstupní hodnoty standardně zachová hodnota existujícího objektu v databázi. Hodnota `null` se automaticky přenese pro Java atributy typů `java.util.Date`, `java.sql.Date`, `LocalDate`, `LocalDateTime` a objektová pole anotovaná jako `NUMBER`, která podporují `null`. Při importu se `NUMBER` přenese pouze tehdy, když je sloupec v importovaném Excel souboru; vynechaný sloupec zachová původní hodnotu a hodnota `NULL` ji vynuluje. Pro datum uložené v jiném typu, například `Instant`, `Timestamp` nebo `Long`, případně pro `TEXT_NUMBER`, nastavte atribut na `true` ; hodnotou `false` automatický přenos vypnete.
- `ai` - ​​nastavením na hodnotu `false` lze vypnout zobrazení AI ikony pro obecné možnosti (přeložit, opravit gramatiku...). AI ikona se zobrazí pouze pokud je asistent nastaven pro toto konkrétní pole.
- `disabled` - ​​nastavením na `false` se vstupnímu poli v editoru nastaví atribut `disabled="disabled"`.
- `export` - ​​nastavením na hodnotu `false` se pole nebude exportovat.

## Vlastnosti @DataTableColumnEditor

- ```type``` - ​​typ ```input``` elementu
- ```label``` - ​​překladový klíč názvu pole v editoru (pokud se liší od ```DatatableColumn.title```)
- ```message``` - ​​překladový klíč pro zobrazení tooltipu, pokud není zadáno automaticky se hledá překlad pro klíč podle ```DatatableColumn.title.tooltip```. Podporuje zadání formátování pomocí základního [Markdown](../frameworks/webjetjs.md#markdown-parser).
- ```tab``` - ​​[karta, ve kterém se pole nachází](README.md#karty-v-editore)
...
- `data-dt-escape-slash` - ​​nastavením na hodnotu `true` zapnete náhradu znaku `/` za entitu `&#47;`. Používá se v případě webové stránky a složky, kde je třeba v názvu znak `/` nahradit, jelikož se používá pro oddělení cesty.
...
- `DURATION` - ​​pole pro zobrazení trvání ve formátu `mm:ss`, vstupem je `long` hodnota v milisekundách
...
- Voláním statické metody pomocí anotace ```@DataTableColumnEditorAttr(key = "method:sk.iway.basecms.contact.ContactRestController.getCountries", value = "label:value")```. V ```key``` atributu je zadána prefixem ```method:``` třídě a metoda, která musí vrátit ```List``` objektů. V atributu ```value = "label:value"``` anotace je zadáno jméno atributu pro popis a jméno atributu pro hodnotu výběrového pole (v příkladu se tedy jmenuje ```objekt.getLabel() a objekt.getValue()```).
- Přidáním anotace `@DataTableColumnEditor.optionMethods` se zadanou metodou k provedení (jedná se o krásnější zápis oproti předchozímu způsobu). Hodnota `labelProperty` a `valueProperty` se použije pro získání textu a hodnoty `option` prvku, pokud není zadána získá se z `label` a `value` atributu.
- Napojením na aplikaci číselníky zadáním ```@DataTableColumnEditorAttr(key = "enumeration:Okresne Mestá", value = "string1:string2")```. V ```key``` atributu je zadán prefix ```enumeration:``` jméno nebo ID číselníku. V atributu ```value = "string1:string2"``` anotace je zadáno jméno atributu pro popis a jméno atributu pro hodnotu výběrového pole - v příkladu se tedy jmenuje ```objekt.getString1() a objekt.getString2()```.
...
- ```@NotNull``` - ​​hodnota nesmí být `null` ; použijte zejména pro objektové typy včetně polí `DATE` a `DATETIME`
- ```@NotEmpty``` - ​​řetězec, kolekce, mapa nebo pole nesmí být `null` ani prázdné; anotace není určena pro typ `Date`
- ```@NotBlank``` - ​​text nesmí být `null`, prázdný ani obsahovat pouze mezery nebo tabulátory

Pro datová pole použijte výhradně `@NotNull`. Frontend odešle prázdnou hodnotu jako prázdný řetězec, který server při deserializaci převede na `null`, aby ji standardní Bean Validation vyhodnotila jako chybějící povinnou hodnotu. Editor pro `DATE` a `DATETIME` zobrazí typově specifické chybové hlášení.

Další možnosti validace jsou popsány v dokumentaci k [restcontrolleru](../datatables/restcontroller.md#validace--povinné-pole).

## Validace

Podporovány jsou standardní anotace pro validace polí pomocí [jakarta.validation.Validator](https://www.baeldung.com/javax-validation). Příklad validace délky pole:
...
Soubor ```app-init.js``` obsahuje funkci ```WJ.DataTable.mergeColumns``` pro doplnění vlastností, na základně názvu objektu (pole name).
Přeiteruje pole objektů columns, najde objekt se stejným názvem jako argument objekt a pomocí ```jQuery.extend``` tento objekt rozšíří.
...
Pokud potřebujete změnit pořadí sloupců, můžete to provést pomocí funkce ```WJ.DataTable.moveColumn```. V příkladu se sloupec s názvem ```formSettings.recipients``` přesune za sloupec s názvem ```formName```:
...
takto anotovaný atribut bude prohledán k anotaci ```@DatatableColumn``` rekurzivně. Výsledkem bude vygenerování vnořené anotace. Všimněte si data a názvu atributu obsahující prefix ```editorFields.```. Pokud potřebujete nastavit vlastní prefix (jméno proměnné) můžete použít parametr prefix ```@DataTableColumnNested(prefix = "menoPremennej")```. Prefix můžete nastavit i na prázdnou hodnotu, tehdy se prefix nevygeneruje.
...
Pokud jako vnořený atribut máme jinou entitu, je vhodné nastavit i atribut `sortPrefix` na hodnotu jména atributu. Automaticky se na atributech vnořené entity přidá tento prefix do pole `sortAfter`, jinak by se vám smísily názvy polí z různých entit.
...
V případě potřeby je možné zadat speciální hodnotu `sortAfter = "FIRST"` pro přesun pole na začátek seznamu. To je nutné použít v případě rozšířených entit přes `@MappedSuperclass` i první atribut `id` je v této entitě.
...
Když atribut ```title``` nezadáte, automaticky se hledá překladový klíč ve formátu ```components.meno_beanu_bez_dto_alebo_bean_na_konci.name```, tzn. ```components.monitoring.date_insert```.
