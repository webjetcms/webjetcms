...
- ```sourceMapPathOverrides``` - ​​nastavení hledání zdrojových kódů vůči ```.map``` souborům.

Zde je důležité si uvědomit, že pokud jsou JavaScript soubory kompilované, je třeba, aby se generoval i ```.map``` soubor. Ten ale typicky nemá přesně nastavenou cestu ve vašem vývojovém prostředí, proto v konfiguraci ```sourceMapPathOverrides``` lze nastavit nahrazení/doplnění cesty na absolutní.

Aby ```rspack``` generoval i ```.map``` soubor je třeba upravit konfiguraci nastavením atributu ```devtoolModuleFilenameTemplate```:
...
Pokud se používá zkompilovaný ```ninja.js``` (jako např. v šabloně ```bare```, obsahující i jQuery a bootstrap) a zároveň je to první soubor v seznamu, tak ```combine``` pro tento soubor negeneruje úvodní komentář a čísla řádků pak sedí vůči ```.map``` souboru, čili pro tento případ @@ není.
...
Pokud používáte multidomain nezapomeňte správně nastavit parametr ```-Dwebjet.showDocActionAllowedDocids``` v ```build.gradle``` pro možnost otevření web stránky zadáním jejího ```docid```.
...
V ukázkovém souboru jsou 2 Java konfigurace - jedna pro databázové připojení v souboru ```poolman.xml``` a jedno pro připojení v souboru ```poolman-local.xml``` (tj. např. do vaší lokální databáze). Soubor ```src/main/resources/poolman-local*.xml``` je v ```.gitignore``` a může jej mít každý programátor nastaven podle svých potřeb. Konfigurace ```Debug (Attach)``` je určena k dodatečnému připojení se k debug portu po spuštění projektu (nebo po jeho spuštění z terminálu).
