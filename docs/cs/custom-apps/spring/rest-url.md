...
Nastavením konfigurační proměnné `logoffRequireCsrfToken` na hodnotu `true` lze aktivovat vyžadování CSRF tokenu pro odhlášení uživatele (z administrace i zákaznické zóny).
...
CSRF ochranu lze aktivovat na libovolné URL adresy, například `/private/rest` nastavením konfigurační proměnné `csrfRequiredUrls` do které zadáte na nový řádek začátky URL adres pro které má být CSRF ochrana vyžadována. Podporován je také formát se znaky `%` pro obsahuje a `!` pro končí na. Příklad:
