# Databázový uživatel s právy pouze ke čtení

V režimu [cluster](README.md#cluster) můžete pro veřejně dostupný uzel zvýšit bezpečnost nastavením databázového uživatele s právy pouze pro čtení vybraných (citlivých) tabulek. Kromě korektně nastavených práv na databázové tabulky nastavte pro tento uzel následující konfigurační proměnné:

- `clusterMyNodeType` - ​​nastavte na `public`, aby tento uzel běžel bez administrace a omezil některé zápisy do tabulek.
- `monitoringEnableCountUsersOnAllNodes` - ​​nastavte na `false` aby nedocházelo k zápisu do tabulky s konfigurací. Více v [monitoringu serveru](../../sysadmin/monitoring/README.md).

## Práva na tabulky

Příklad nastavení práv pro uživatele `WEB_WJ_APP` s právy pouze pro čtení pro Oracle databázi. Uživatel `WEB_WJ` je uživatel s plnými právy a vlastník schématu s daty.
...
Příklad pro MariaDB, jméno schématu je `webjetcms_web`, uživatel s právy na čtení je `webjetcms_web_ro`:
...
Pro Oracle databázi je třeba vytvořit `trigger` pro změnu schématu po přihlášení a nastavit proceduru pro vytvoření statistických tabulek.
