...
WebJET obsahuje integrovanou ochranu proti SPAMu, při které limituje rychlost požadavků a jejich počet, aby nedošlo k přetížení serveru. Taková ochrana se označuje také jako `Rate Limiting`, čili limitování počtu požadavků.

Ochrana je vázána na IP adresu návštěvníka, tedy limity platí pro každou IP adresu samostatně. Je tedy třeba mít správně nastaveno získávání IP adresy návštěvníka. Pokud je například před aplikačním serverem předřazen `Load Balancer/Proxy`, je nutné nastavit konfigurační proměnnou `serverBeyoundProxy` na hodnotu `true` a na `Load Balancer/Proxy` nastavit HTTP hlavičky `x-forwarded-for` s IP adresou návštěvníka a `x-forwarded-proto` s hodnotou použitého protokolu @@CODE_. Ověřit správnost IP adresy můžete v [auditě](../../sysadmin/audit/README.md) ve sloupci IP adresa při provedení nějaké akce, například přihlášení, nebo odeslání formuláře.
...
- `spamProtectionIgnoreFirstRequests` - ​​počet prvních požadavků z IP adresy, které nejsou limitovány, ve výchozím nastavení nastaveno na 0, čili bez výjimky limitování.

### Nastavení pro modul/aplikaci

Limity lze upravit specificky pro některé moduly/aplikace přidáním `-appName` na konec konfigurační proměnné. `spamProtectionHourlyLimit-appName` nebo `spamProtectionTimeout-appName`. Nastavit je hodnoty možné pro následující aplikace:
...
- `404` - ​​[počet volání stránky 404](../../sysadmin/troubleshooting/README.md), limituje kolikrát je možné volat stránku 404, po překročení limitu je zrušena `session` a uživatel je odhlášen.
