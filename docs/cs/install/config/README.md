...
K logování se používá knihovna [Logback](http://logback.qos.ch/), která umožňuje detailní nastavení logování pro jednotlivé java balíky. Používá se [ConsoleAppender](https://logback.qos.ch/manual/appenders.html#ConsoleAppender), lze ale nastavit i [FileAppender](https://logback.qos.ch/manual/appenders.html#FileAppender) nebo [RollingFileAppender](https://logback.qos.ch/manual/app) Konfigurační soubor je v `WEB-INF/classes/logback.xml`, avšak pro základní nastavení lze použít následující konfigurační proměnné:
...
- `smtpUseTLS` - ​​`TLS` autentifikace - pokud je port 587, `smtpUseTLS` musí být `true`.
- `smtpTLSVersion` - ​​verze `TLS` pro smtp spojení.
- `smtpAuthMechanism` - ​​autorizační mechanismus pro SMTP server (např. `NTLM XOAUTH2`), je-li prázdný, použije se výchozí mechanismus knihovny JavaMail. Nastavuje hodnotu `mail.smtp.auth.mechanisms`. Pro vynucení použití `NTLM/v2` před `BASIC` nastavte hodnotu na `NTLM`. Pro podporu `OAuth` nastavte hodnotu na `XOAUTH2`.
...
- `formmail` - ​​odesílatel notifikace pro návštěvníka, který vyplnil formulář.
...
Pro hromadný email doporučujeme použít službu [Amazon Simple Email Services/SES](https://aws.amazon.com/ses/) pro lepší doručování emailů. Původně WebJET CMS používal API přístup, který se aktivoval nastavením konf. proměnné `useAmazonSES` na hodnotu `true`, aktuálně již je ale používán [standardní SMTP protokol](https://docs.aws.amazon.com/ses/latest/dg/send-email-smtp.html) ve službě Amazon SES:
...
Umí běžet v módu `auto`, kdy nepotřebuje seznam uzlů, nebo v režimu, kdy je mě přesně jmenováno.
...
Hodnota je zkrácena na prvních 16 znaků. Pokud je proměnná `clusterHostnameTrimFromEnd` nastavena na `true`, použije se koncových 16 znaků (např. kubernetes vytváří `hostname` s náhodnou hodnotou na konci).

Konfigurační proměnná `clusterAutoRandomDelay` nastavuje maximální čas náhodného zpoždění v milisekundách přidaný v režimu `clusterNames=auto` při startu `ClusterRefresher`, `Sender` a před spuštěním `CRON` úloh. Pomáhá rozložit start úloh mezi uzly clusteru a snížit riziko databázového `deadlock`. Hodnota `0` zpoždění zcela vypne.
...
Jednotlivým uzlem potřebujete externě definovat `ID` uzly, nemůžete to udělat přes Nastavení->Konfigurace, protože by všechny uzly měly stejný název.
...
- `clusterRefreshTimeout` - ​​interval synchronizace uzlů v ms, ve výchozím nastavení `5000ms`. Pro produkci obvykle postačuje hodnota 60000 (1 minuta) pro snížení zatížení.
...
Aby nedocházelo ke konfliktu v cluster konfiguraci používá se hodnota `pkeyGenOffset` pro posun podle nodů. Např. hodnota `pkeyGenIncrement` je nastavena na 5 a `offset` na 0-5 pro jednotlivé uzly.

Při režimu `auto` clustra nelze použít `pkeyGenOffset`, proto se alokuje zadaný blok v transakci a pokud nastane `deadlock`, pokus se opakuje maximálně 5krát s exponenciálně rostoucím zpožděním od 50 ms (`50 * 2^attempt`) as náhodnou složkou (jitter).
