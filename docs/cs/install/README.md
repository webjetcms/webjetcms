...
Podobně pro verzi Maven Central je třeba přidat příkaz `dependencies` bloková část `implementation("sk.iway:webjet:${webjetVersion}:libs")`:
...
- V Maven Central se název balíčků změní z `sk.iway` na adrese `com.webjetcms` a doplňuje se oddíl `libs` která kombinuje všechny `sk.iway` Závislost. V `build.gradle` upravit:
