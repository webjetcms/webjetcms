...
Výrazný vliv na výkon mohou mít také vyhledávače, které neustále procházejí a indexují web stránky na vašem serveru. Jejich návštěvnost nemusí být vidět například. v Google Analytics, ale je vidět ve [statistice](../../redactor/apps/stat/README.md), kterou poskytuje WebJET CMS.
...
Pomocí URL parametru `?_writePerfStat=true` lze získat výpis aplikací vložených ve web stránce s časem jejich provedení. Například stránku `/sk/` zobrazíte jako `/sk/?_writePerfStat=true`.
...
Existuje aplikace `/components/_common/generation_time.jsp` kterou pokud vložíte do patičky šablony web stránky vygeneruje do HTML kódu celkový čas generování web stránky.

Lze nastavit následující parametry aplikace:

- `hide` - ​​výchozí `true` - čas generování se zobrazí jako komentář v HTML kódu
- `onlyForAdmin` - ​​ve výchozím nastavení `false` - doba generování se zobrazí pouze pokud je přihlášen administrátor
...
Z důvodu rozdílného počtu záznamů v databázi je třeba porovnávat `per item` hodnoty.
...
Každá web stránka má v kartě Základní možnost **Povolit uložení stránky do vyrovnávací paměti**. Zapnutím této možnosti se obsah webové stránky z tabulky `documents` uloží do vyrovnávací paměti. Při zobrazení web stránky nebude nutné provést databázové volání pro získání obsahu web stránky.
...
Pro přesnější nastavení lze použít aplikaci [HTTP záhlaví](../../admin/settings/response-header/README.md), kde můžete nastavit rozdílné hodnoty pro rozdílné URL adresy.
...
Chování robotů lze ovlivnit nastavením v souboru `/robots.txt`. Tento pokud neexistuje je generován ve výchozím stavu. Vámi upravenou verzi umístěte do `/files/robots.txt`, z této lokality jej WebJET zobrazí při volání `/robots.txt`.
...
Nastavením konfigurační proměnné `disableReverseDns` na hodnotu `true` lze vypnout získávání DNS názvu z IP adresy návštěvníka a zrychlit provádění požadavků. Do pole pro hodnotu `hostname` se tehdy zapíše hodnota IP adresy.
