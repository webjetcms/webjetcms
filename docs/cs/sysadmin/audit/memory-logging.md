...
- ```loggingInMemoryQueueSize``` - ​​maximální počet logů zapsaných do paměti (výchozí 200). Upozorňujeme, že všechna data jsou načtena najednou do tabulky az důvodu přenosu ```stack trace``` mohou být velká. Nedoporučujeme tuto proměnnou nastavovat na extrémně vysokou hodnotu.
...
- ```sk.iway.iwcm.system.logging.InMemoryLoggingDB``` - ​​třída zabezpečuje zápis a získání logů za do ```queue```, načítání logů na clusteru
