...
Aplikace Audit je nástroj pro sledování změn v systému. Sekce Audit -> Vyhledávání umožňuje zobrazit a filtrovat jednotlivé zaznamenané hodnoty.
Filtrování je možné podle typu auditních záznamů, času, uživatele atp.
...
- ```CRON``` - ​​zaznamená běh úloh na pozadí, pokud je zaškrtnuta možnost Auditovat. Ukládá i chyby při provádění úkolů (pokud nastanou), v takovém případě zaznamená ```Stack Trace```.
...
- ```FORMMAIL``` - ​​odeslání formuláře. Zaznamená úspěšné odeslání s hlášením ```FormMail formName:``` jméno formuláře, seznam příjemců a ```referer```. Při neúspěchu zaznamená důvod ne odeslání s hlášením ```ERROR: formName:``` jméno formuláře, ```fail:``` důvod ne odeslání. Zaznamenává také detekci spamu hlášením ```detectSpam TRUE:``` důvod detekce jako spam.
