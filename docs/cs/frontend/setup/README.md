...
Takové nastavení zajistí oddělení dat a souborů jednotlivých domén. Pokud potřebujete sdílet soubory mezi doménami můžete použít složku ```/shared```, která je mezi doménami sdílena.
...
WebJET 2021 zobrazuje složky v seznamu webových stránek pouze pro zvolenou doménu. Při uložení kořenové složky domény se automaticky vytvoří lokální ```System``` složku pro stránky hlaviček, patiček, menu atp. Při uložení kořenové složky domény je proto nutné obnovit celou stránku, aby se korektně načetl odkaz na složku ```System```.

Složka ```System``` je na rozdíl od WebJET 8 uložena v kořeni (můžete ji zobrazit ve struktuře složek kliknutím na list Složky při stisknuté ```shift``` klávese) a ne v prvním adresáři zvolené domény (pokud ale existuje v doménovém adresáři, použije se ten).
