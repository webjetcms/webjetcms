...
- `virtualPathLastSlash` ve výchozím nastavení `true` - nastavuje možnost posledního `/` pro URL adresy **hlavní stránky**. Při nastavení na `true` bude URL adresa stránky Produkty vytvořena jako `/products/`, při nastavení na `false` jako `/products`.
...
- `editorTargetBlankFunction` výchozí `return openTargetBlank(this, event)` - jméno JavaScript funkce, která se použije místo target=\"_blank\" a target='_blank' jako onclick.
