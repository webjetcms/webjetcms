...
Základní ukázková šablona pro WebJET CMS s využitím [Thymeleaf šablon] (http://docs.webjetcms.sk/v2022/#/frontend/thymeleaf/README) napsaných ve formátu [PugJS](http://docs.webjetcms.sk/v2022/#/de/ Je založena na [Start Bootstrap - Bare](https://startbootstrap.com/template/bare/).
...
Všechny cesty, které píšete do ```.pug/.scss/.js``` souborů používáte včetně prefixu ```/templates/bare/....```, čili jakoby pro použití ve WebJETu. Pokud spustíte lokální verzi přes ```npm run start```, cesty ale mají prefix ```/``` (jelikož kořenová složka je vlastně ```dist```). V souboru ```node_scripts/bs-config.js``` je nastaveno přesměrování, pokud změníte cestu k šabloně nezapomeňte ji upravit iv tomto souboru.
...
Pokud již doménu/strukturu stránek máte vytvořenou, doporučujeme upravit existující složky - v kartě Šablona složky nastavte ```Bare - Hlavná šablóna``` a zapněte možnost ```Aplikovať na všetky existujúce podpriečinky a podstránky```. Toto nastavte také v kartě ```Systém``` na existující složky.
...
Podobně postupujte pro nastavení patičky (stránka ```Základná pätička```), smažte vše a vložte blok ```footer-footer```. Kromě standardních textových sloupců, kde můžete jednoduše upravit text podle vašich potřeb, obsahuje patička aplikace:
...
Šablona je připravena pro jazykové mutace, využívá aplikaci Zrcadlení struktury. Doporučujeme v seznamu webových stránek (karta Složky) vytvořit kořenové složky ```Slovensky``` a ```English```.

Složkou nastavte pole ```URL adresa``` na hodnotu ```sk``` nebo ```en```, rovněž nastavte ```Názov položky v menu``` na hodnotu ```sk``` nebo ```en``` (tato položka se zobrazí v hlavičce v přepínači jazyků) av kartě Šablona pole ```Jazyk``` na Následně postupujte podle návodu pro [nastavení zrcadlení](http://docs.webjetcms.sk/v2022/#/redactor/apps/docmirroring/README).
...
jednotlivé ```npm``` skripty jsou definovány v [package.json](https://github.com/webjetcms/templates-bare/blob/master/package.json) v elementu ```scripts``` a provedeny z ```node_scripts/*.js``` adresáře:
...
Pokud ```editor.css``` nevkládáte i do designu šablony, tak se jeho styly při zobrazení web stránky neaplikují. Standardně se tedy používá jen jako seznam definic stylů.
...
Doporučujeme postupovat ze spodu, tedy nejprve vytvořit bloky v ```column``` složce, ty následně pomocí include vložit do ```container``` a to následně do ```section```.
...
výhoda takového postupu je v tom, že pokud upravíte něco v ```column``` pug souboru, projeví se díky použití ```include``` příkazu změna iv ```container``` i v ```section``` verzi.
...
Generování vyžaduje nainstalovaný [PhantomJS](https://phantomjs.org/download.html), umístění a nastavení konf. proměnných je v souboru ```localconf.jsp```.

## Copyright and License

Copyright 2013-2021 Start Bootstrap LLC. Code released pod [MIT](https://github.com/StartBootstrap/startbootstrap-bare/blob/master/LICENSE) license.
