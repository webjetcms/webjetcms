...
Vloží skript s linkem na aktuální verzi jquery, která se ve webjetu používá. Pokud ji nevložíš tímto způsobem, vloží se stejně sama. Tímto způsobem alespoň umíš určit vísto, kam se má volání vygenerovat.
Pokud nechceš, aby se jquery přidalo, zavolej metodu `${ninja.webjet.insertJqueryFake}`. Ona to nasimuluje a jquery se nepřidá.
...
Nasimulování vložení jquery verze. Tím se blokuje automatické vložení aktuální jquery knihovny na web. Potom je třeba ručně přidat volání globálních JS funkcí pomocí `${ninja.webjet.pageFunctionsPath}`.
