...
**Volitelná pole**
Přidána možnost [výběrové pole s více možnostmi](frontend/webpages/customfields/README.md#výberové-pole-viac-možností) zadáním předpony `multiple:` před seznamem možných hodnot. Hodnoty se pak uloží do pole odděleného znakem `|`.
...
**Datové tabulky**
- Přidán nový typ datového pole pro field type číselníku `select` s možností [editace nebo přidání záznamu](developer/datatables-editor/field-select-editable.md) ve výběrovém poli. Umožňují snadno upravit objekt, který je v poli vybrán, například šablonu stránky, odkaz na záhlaví stránky apod.
...
- Doplněný manuál pro redaktora o aplikacích [Menu](redactor/apps/menu/README.md), [Navigační panel](redactor/apps/navbar/README.md) a [Mapa stránek](redactor/apps/sitemap/README.md).
...
Použití je snadné rozšířením repozitář Spring DATA ze třídy `DomainIdRepository<T, ID>`, o zbytek se postará přímo `DatatableRestControllerV2`. Více informací najdete přímo v dokumentaci [Oddělení dat domény](developer/datatables/domainid.md).
...
- Přepracované vyhledávání při použití typu repozitář Spring DATA `JpaSpecificationExecutor`. V tomto případě se vyhledávání vytvoří na základě `HTTP request` parametry začínající výrazem `search`. Již neplatí `ExampleMatcher` ale hledaný výraz je porovnáván na základě parametrů. To má tu výhodu, že v objektu lze použít primitivní i neprimitivní parametry. `NULL` atributy (které v případě `ExampleMatcher` automaticky zahrnuty do vyhledávání).
**Obecné**
- Přidána funkce JS `WJ.htmlToText` pro konverzi [Převod kódu HTML na text](developer/frameworks/webjetjs.md#ostatné-funkcie).
**Zabezpečení**
...
- Vytvořená dokumentace o možnosti použití [dědičnost v entitách](developer/datatables/restcontroller.md#dedenie-v-jpa-entitách).
- Dokumentace vytvořená pro použití [dědičnost v entitách JPA](developer/datatables/restcontroller.md#dedenie-v-jpa-entitách).
...
- Vytvořená funkce [WJ.breadcrumb](developer/frameworks/webjetjs.md#navigačná-lišta) k vygenerování navigačního panelu.
...
- Exportovány jsou také seznamy adresářů/stránek/adresářů v souborovém systému, které používají field type. `json`. Exportované i importované soubory jsou v zobrazení celé cesty (např. `/Slovensky/Novinky`).
...
- Pro pole typu JSON [vybrat jednu stránku nebo adresář](developer/datatables-editor/field-json.md#možnosti-classname) přidána možnost odstranit nastavenou hodnotu nastavením typu pole na hodnotu `dt-tree-group-null` pro adresář nebo `alebo dt-tree-page-null` pro webovou stránku.
...
- Přidána možnost [vynucené zobrazení sloupců](developer/datatables/README.md#možnosti-konfigurácie) (např. pro vnořenou datovou tabulku) nastavením konfigurační možnosti `forceVisibleColumns`.
- Přidána možnost volat funkci JavaScriptu pro [upravit seznam sloupců](developer/datatables/README.md#možnosti-konfigurácie) (např. pro vnořenou datovou tabulku) nastavením konfigurační možnosti `updateColumnsFunction`.
- Přidána možnost [přidání stylu CSS s přepínačem karet](developer/datatables-editor/README.md#karty-v-editore) v editoru pomocí atributu `className`, pomocí stylu CSS `hide-on-create` je možné kartu při vytváření nového záznamu skrýt.
- Přidána možnost [skrýt kartu na základě práv](developer/datatables-editor/README.md#karty-v-editore) v editoru pomocí atributu `perms`.
...
- Přidaný field type `jstree` zobrazit [stromové struktury](developer/datatables-editor/field-jstree.md) s výběrovými poli uzlů stromu.
- Field type `json` rozšířeno o možnost [výběr seznamu adresářů souborového systému](developer/datatables-editor/field-json.md).
- Přidaná pole `HIDDEN, RADIO, PASSWORD` Pro `DatatableColumnType`.
- Zjednodušený proces práce s [další/vložené atributy](developer/datatables-editor/datatable-columns.md#vnorené-atribúty), `editorFields` v řadiči REST. Implementací metod `processFromEntity` a `processToEntity` zajistit správné mapování dat mezi entitou a `editorFields` objekt.
- Import - přidány podmínky pro `domainId` aby se při importu aktualizovaly pouze záznamy aktuální domény.
- Import - upravený import s odpovídajícími záznamy podle vybraného sloupce pro použití API `insertItem` a `editItem` aby import fungoval i pro rozhraní REST, která nepoužívají repozitář Spring DATA.
...
- Přidána možnost [zobrazit kód HTML](developer/datatables/README.md#zobrazenie-html-kódu) v buňce nastavením stylu CSS `allow-html` (#53989).
...
- Přidáno načtení všech dat řádku ze serveru před úpravou buňky, protože samotná datová tabulka nemusí vždy obsahovat všechna data potřebná pro redaktora (#54109).
**Zabezpečení**
...
- Dokumentace vytvořená pro [Překladač](admin/setup/translation.md).
...
- Přidána dokumentace o možnosti rozšíření pole typu `json` o [vlastní verze](developer/datatables-editor/field-json.md#vlastná-konfigurácia-zobrazenej-stromovej-štruktúry) struktury jsTree.
...
- Datové tabulky - opraveno nastavení nepovinných polí v nově vytvořeném záznamu.
...
**Datové tabulky**
- Přidána možnost `onRowCallback` pro možnost [stylování řádků](developer/datatables/README.md#štýlovanie-riadku-a-ikony) na straně klienta (např. označení řádku jako neaktivního).
...
- Přidána možnost generovat [allure](developer/testing/README.md#generovanie-html-reportu) Zprávy.
...
- Přepracované ověřování polí pomocí anotací na [překladatelské klíče z WebJET](developer/datatables/restcontroller.md#validácia--povinné-polia). Upravil zobrazované chybové zprávy na formát "Co způsobilo chybu. Co má uživatel udělat".
...
- Doplněná dokumentace k označení [povinná pole](developer/datatables-editor/datatable-columns.md#povinné-polia) v anotacích.
- Rozšířená dokumentace o [validace pomocí anotací](developer/datatables/restcontroller.md#validácia--povinné-polia).
...
Připravili jsme dokumentaci s informacemi o [nastavení nepovinných polí](frontend/webpages/customfields/README.md) pro frontendového programátora a dokumentaci s [technické informace](developer/datatables-editor/customfields.md) pro backendového programátora.
...
- Volitelná pole - informace o [nastavení nepovinných polí](frontend/webpages/customfields/README.md) pro frontendového programátora.
...
**Datovatelné**
- Upravit buňku - přidána možnost úprav [jednotlivé buňky](developer/datatables-editor/bubble.md) přímo v datové tabulce i pro redaktora obsahující samostatné listy (dosud to fungovalo pouze v editoru bez listů). Přidána možnost nastavit needitovatelné buňky.
...
**Správa**
- Přidáno [zveřejnění události](developer/frameworks/thymeleaf.md#vloženie-vlastných-objektov-do-modelu), `ThymeleafEvent` při zobrazení stránky pro správu. Umožňuje dynamicky vkládat objekty do modelu stránky.
**Překlady**
Zavedený framework pro [překlad textů v souborech JavaScript](developer/frameworks/jstranslate.md). Skládá se z `REST` služba poskytující mapu překladu klíčů a textů a knihovna JavaScriptu poskytující překlad na základě klíčů. Knihovna také zajišťuje, aby byly překladové klíče uloženy v paměti prohlížeče a aktualizovány při změně textu překladu (#53128).
...
Během implementace bylo vytvořeno několik knihoven pro zobrazování protokolů v konzole prohlížeče, přístup k `storage` objekt prohlížeče a spouštění událostí. Ve výchozím nastavení již není možné použít událost `$(document).ready`, protože je třeba nejprve inicializovat repozitář překladových klíčů. Implementovali jsme vlastní funkci `window.domReady.add`, která se provede až po inicializaci repozitář překladových klíčů.
...
- Přidána možnost automatického [skrýt některá tlačítka](developer/datatables/README.md#možnosti-konfigurácie) na panelu nástrojů datové tabulky nastavením možnosti `hideButtons`.
...
- Zobecněné [vyhledávání podle uživatelského jména](\(developer/datatables/restcontroller.md#zobrazenie-mena-používateľa-a-vyhľadávanie\)) při ukládání pouze ID do databáze.
...
**Obecné**
- Přidána možnost použít `autocomplete` Také [mimo datové tabulky](developer/datatables-editor/autocomplete.md#použitie-mimo-datatabuľky).
...
- Dokumentace vytvořená pro nové knihovny [Překladač](developer/libraries/Překladač.md), [StorageHandler](developer/libraries/storage-handler.md), [Obchod](developer/libraries/store.md), [Nástroje](developer/libraries/tools.md), [ReadyExtender](developer/libraries/ready-extender.md).
- Dokumentace vytvořená pro [optimalizace rychlosti zobrazení](developer/apps/webpages/README.md) seznam webových stránek.
- Doplněná dokumentace k datové tabulce - doplněná [seznam možností konfigurace](developer/datatables/README.md#možnosti-konfigurácie).
- Vytvořená dokumentace o možnosti [vkládání vlastních objektů](developer/frameworks/thymeleaf.md#vloženie-vlastných-objektov-do-modelu) k modelu v `Thymeleaf` na zadní straně.
- Přidána dokumentace ke knihovně jstree, která obsahuje informace o vkládání. [inicializační data](developer/jstree/README.md#konfigurácia).
- Přidána dokumentace o vyvolání obecné chyby v datové tabulce a o [zabránit úpravám nebo vymazání](developer/datatables/restcontroller.md#zabránenie-zmazania--editácie-záznamu) záznamu.
- Doplněná dokumentace pro [zobrazení a vyhledávání podle uživatelského jména](developer/datatables/restcontroller.md#zobrazenie-mena-používateľa-a-vyhľadávanie) při ukládání pouze ID do databáze.
...
**Datové tabulky**
- Přidaná funkce `$.fn.dataTable.Buttons.showIfOneRowSelected(this, dt);` zobrazit tlačítko, když je přesně označeno [jeden řádek](developer/datatables/README.md#pridanieodobratie-tlačidiel).
...
- Rozšířená dokumentace o přidávání [datová tlačítka](developer/datatables/README.md#pridanieodobratie-tlačidiel).
...
- Přidána možnost programově [odebrat práva](developer/testing/README.md#odobratie-práva) aktuálně přihlášenému uživateli (platí pouze pro uživatele s přihlašovacím jménem `tester`).
- Přidána možnost automatizovaného testu datových souborů [kontrola práva](developer/testing/datatable.md#testovanie-práv).
...
- Přidaná funkce `DT.checkPerms(perms, url)` Pro [kontrola práv](developer/testing/README.md#odobratie-práva) Volání služby REST.
- Přidaná funkce `I.jstreeNavigate(pathArray)` Pro [postupná kliknutí](developer/testing/README.md#webjet-doplnkové-funkcie) na zadané názvy.
...
- Přidána dokumentace k funkci [WJ.dispatchEvent](developer/frameworks/webjetjs.md#ostatné-funkcie).
- Přidána dokumentace k datovému poli [datová tabulka](developer/datatables-editor/field-datatable.md).
- Doplněná dokumentace pro [možnosti nastavení](developer/testing/datatable.md#možnosti-nastavenia) DATATabulky.
- Přidána dokumentace k možnosti [odnětí práv](developer/testing/README.md#odobratie-práva) během testování.
...
**Obecné**
- Přidali jsme funkce WJ do [formátování data a času](developer/frameworks/webjetjs.md#formátovanie-dátumu-a-času) v jazyce JavaScript (#52999).
...
- Přidána dokumentace k třídám CSS [zakázat zapnutí/skrýt zapnutí](developer/datatables-editor/README#možnosti-konfigurácie-columns-objektu) pro redaktora
- Dokončená dokumentace údržby [připojení k serveru](developer/frameworks/webjetjs.md#udržiavanie-spojenia-so-serverom-refresher) (#53119)
...
- Datatable-editor - pevné vkládání řádků (`data-dt-field-hr`) pro redaktora bez záložek (listů) - při druhém otevření editoru se nezobrazovala dělící čára(#52999).
...
- Přidána sekce [Best practices](developer/testing/README.md#best-practices) pro testování.
...
- Přidáno [podpora vizuálního testování](developer/testing/README.md#vizuálne-testovanie) (porovnání obrázků), výsledkem je vizuální zvýraznění vadné oblasti růžovou barvou: ![](developer/testing/autotest-insert-script-settings-diff.png)
- přidána funkce pro snadné nastavení výběrového pole filtru datatable [DT.filterSelect(name, value)](developer/testing/README.md#webjet-doplnkové-funkcie)

**Dokumentace**

- Testy - přidána dokumentace k možnosti [spuštění konkrétního scénáře](developer/testing/README.md#spustenie-testovania)
- Testy - doplněná dokumentace pro [podpora vizuálního testování](developer/testing/README.md#vizuálne-testovanie)
- Deployment - přidali jsme popis činností, které je třeba provést před deployment. [nové verze](developer/install/deployment.md).
...
- dokumentace k zahájení dokončena [dialogové okno iframe](developer/frameworks/webjetjs.md#iframe-dialog)
...
> 2021.1 přináší editaci adresářů webových stránek a aktualizovaný testovací framework
...
- doplněná dokumentace pro [pomocí editorField](developer/datatables-editor/datatable-columns.md#vnorené-atribúty)
- doplněná dokumentace pro [stylování čar a ikon](developer/datatables/README.md#štýlovanie-riadku-a-ikony)
- dokumentace o možnosti [vyvolání chybové zprávy](developer/datatables/restcontroller.md#validácia--povinné-polia) při úpravě položky (metoda `throwError`)
...
**Datové tabulky**
- #52396 - Přidána možnost stylovat řádky a přidávat další ikony, [Dokumentace](developer/datatables/README.md#štýlovanie-riadku-a-ikony).
...
- #52396 - přidána dokumentace k použití ikon v aplikaci [jstree](developer/jstree/README.md#spôsob-použitia-ikon) a [datová tabulka](developer/datatables/README.md#štýlovanie-riadku-a-ikony)
