...
V seznamu uživatelů je možné spravovat administrátory (vyžaduje právo ```správa administrátorov```) a registrované uživatele web sídla/uživatelů pro hromadný e-mail (vyžaduje právo ```správa registrovaných používateľov```).
...
- Kontaktní údaje - obsahuje údaje uživatele jako Adresa, Město, Telefon a pod.
...
Standardně administrátor, který nemá zvolen žádný adresář souborového systému, může nahrávat soubory do libovolného adresáře. Chování lze změnit nastavením konfigurační proměnné ```defaultDisableUpload``` na hodnotu ```true```, což způsobí, že nahrávat soubory bude možné pouze do zvolených adresářů (a pokud uživatel nemá zvolený žádný adresář nebude moci nahrávat soubory vůbec).
...
V kartě **Schvalování** lze definovat akci při změně ve zvoleném adresáři web stránek (typicky schvalování). Karta se zobrazí pouze pro již uloženého uživatele (nově vytvářeného uživatele je třeba nejprve uložit až následně nastavit proces schvalování).
