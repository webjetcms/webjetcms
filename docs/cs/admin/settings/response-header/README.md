...
- **URL adresa** určuje, pro které URL adresy se definuje hlavička. Podporován je následující zápis:
    - `/folder/subfolder/` - ​​záhlaví se generuje pro všechna URL, která začínají na zadanou hodnotu.
...
Pro URL adresy začínající na `/files,/images,/shared` se automaticky nastavuje HTTP hlavička `Content-Language` podle základního jazyka administrace v konf. proměnné `defaultLanguage`. Navíc jsou nastaveny podle následujících pravidel:
...
Země na základě jazyka se získává z konf. proměnné `countryForLng`, pokud není zadaná použije se jako země stejná hodnota jako požadovaný jazyk.