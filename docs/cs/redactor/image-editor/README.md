...
- ![](tie-btn-resize.png ":no-zoom"), **Změnit velikost**: můžete upravit velikost obrázku. Upozorňujeme, že ve výchozím nastavení je obrázek zobrazen na maximální velikost 1000 bodů na šířku, takže úprava rozměru nemusí být viditelná, protože obrázek může být reálně větší než zobrazená velikost.
  - Velikost lze změnit na základě definovaných šablon velikostí (pokud opakovaně potřebujete nastavit přesný rozměr obrázku). Seznam možností se nastavuje v konfigurační proměnné `imageEditorSizeTemplates`, ve výchozím nastavení nastaveno na hodnotu `80x80;640x480;800x600;`.
...
- ![](tie-btn-crop.png ":no-zoom"), **Oříznout**: Po kliknutí na tuto funkci můžete oříznout fotografii v galerii.
  ...
  - Poměry stran lze definovat v konfigurační proměnné `imageEditorRatio`, ve výchozím nastavení nastavené na hodnotu `3:2, 4:3, 5:4, 7:5, 16:9`.
...
- ![](tie-btn-draw.png ":no-zoom"), **Kreslit**: Tato funkce vám umožňuje kreslit na fotografii volně nebo rovnou čáru. Můžete si vybrat barvu a tloušťku čáry (rozsah).
...
- ![](tie-btn-filter.png ":no-zoom"), **Filter**: Použitím této funkce můžete aplikovat na fotografii různé filtry, které upraví její vzhled. Mezi filtry, ze kterých lze vybírat, patří: černobílý filtr, sépie, rozmazání, ražení, negativ, zaostření. Nastavit umíte odstranění bílé, jas, šum, zrnitost a filtr barev. Dále lze nastavit Tint, Multiply a Blend pro každou barvu.
