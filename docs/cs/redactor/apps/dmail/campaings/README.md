...
Výhodou je, že každý e-mail může být personalizován - pokud do těla mailu vložíte značku ```!name!``` místo ní se vloží skutečné jméno příjemce.
...
V kartě **Pokročilé** můžete nastavit pole email zprávy pro zodpovězení, kopii a skrytou kopii.
...
- ochrana proti nevhodnému emailu, email musí splňovat standardní formát **jméno@domena.cz** (speciální výjimka při [Import z xlsx](#import-z-xlsx))
...
!>**Upozornění,** pokud změníte seznam příjemců PO odeslání emailů, výsledná statistika nebude správná a může vzbudit dojem problému při odesíláním.
...
- Uložen - email je uložen, čeká ve frontě na odeslání
- Odeslán - email je odeslán, ve sloupci Datum odeslání je uloženo přesné datum a čas odeslání emailu
...
Manuální přidání nabízí možnost **Přeskočit chybné záznamy**. Zadáte-li jako E-mail ```Test1@test.sk, Test2@test.sk; Test13Wrong Test4@test.sk Test2@test.sk``` a možnost Přeskočit chybné záznamy **je vypnuta**, tak se přidávání příjemců zastaví na první nevyhovující hodnotě a zobrazí se chyba:
...
**Pokud je možnost zapnuta**, tak se nevyhovující hodnoty přeskočí a zobrazí se oznámení s informací které hodnoty a proč se neuložily:
...
Vaše změny se okamžitě projeví v kartě **Příjemci**. Emaily přidané ze skupiny budou vždy obsahovat nějakou hodnotu ve sloupci **Skupiny**. Může nastat situace, že mezi příjemci jsou emaily, které spadají pod nějakou skupinu i když žádná skupina kampani nebyla přidána.
Situace nastává, pokud ručně přidáte email patřící pod nějakou skupinu. WebJET jej automaticky rozpozná a bude zobrazovat ve sloupci **Skupina** všechny skupiny, pod které patří.
...
- Import z xlxs souboru podporuje výjimku formátu emailu. Při manuálním přidání musí mít každý mail formát **jméno@domena.cz**. Pokud však kopírujete emaily např. z aplikace `Outlook`, zkopírovaná hodnota může mít formát ```"Ján Tester <jan_tester@test.com>"```. V případě, že hodnota obsahuje znaky ```<>```, **(přesně v tomto pořadí)**, použije se hodnota mezi nimi. V tomto případě by to byla právě hodnota ```jan_tester@test.com```. Tato hodnota musí mít formát **jméno@domena.cz**.

## Otevření

Na kartě **Otevření** se zaznamená otevření e-mailu příjemcům. Toto je realizováno pomocí vloženého obrázku. Ne všichni příjemci potvrdí načtení obrázku ze serveru, takže seznam není kompletní.

![](opens.png)

##

Karta **Kliknutí** zobrazuje seznam kliknutí na odkaz v emailu. Příjemce může na odkaz kliknout vícekrát, proto může být v tabulce kliknutí zaznamenáno vícekrát od jednoho příjemce.
