...
- Email se odešle.
- Z odpovědi SMTP serveru se vyhodnotí, zda je email odeslán úspěšně (kromě stavového kódu je možné nastavit doplňkové chybové odpovědi v konf. proměnné ```dmailBadEmailSmtpReplyStatuses```)
...
- Nastavené [DKIM](https://www.dkim.org) klíče domény s platným [SPF](https://cz.wikipedia.org/wiki/Sender_Policy_Framework) záznamem. Doporučujeme použít pro odesílání [Amazon SES](../../../../install/config/README.md#nastavení-amazon-ses) a `DKIM` nastavit tam, automaticky se nastaví i `SPF`.
- Nastavený [DMARC](https://dmarc.org) záznam. V DNS vytvořte nový `TXT` záznam pro doménu `_dmarc.vasadomena.sk` s hodnotou minimálně `v=DMARC1; p=none; sp=none`.
...
- ```dmailMaxRetryCount``` - ​​maximální počet opakování odeslání emailu pokud dojde k chybě při odesílání (výchozí 5).
- ```dmailSleepTimeAfterException``` - ​​interval čekání v ms po chybě odesílání, například. pokud SMTP server přestane odpovídat (výchozí 20000),
- ```dmailBadEmailSmtpReplyStatuses``` - ​​seznam čárkou oddělených výrazů vrácených ze SMTP serveru pro které se email nebude znovu pokoušet odeslat (výchozí: Invalid Address,Recipient address rejected,Bad recipient address,Local address contains control or whitespace,Domain ends with dot in string,Domains)
...
- ```disableDMailSender``` - ​​vypne pouze odesílání hromadných emailů (výchozí ```false```).
- ```senderRunOnNode``` - ​​pokud používáte cluster více aplikačních serverů umožňuje nastavit čárkou oddělený seznam jmen ```nodu```, ze kterého budou odesílány hromadné emaily. Upozornění: pokud jsou emaily odesílány z více ```nodov``` může dojít k duplicitnímu odeslání emailu.
