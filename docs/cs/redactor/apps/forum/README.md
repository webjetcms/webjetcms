...
- Nástěnka diskusí - vícetémová diskuse (```Message board```), kde se diskuse dělí na skupiny a pod-témata, která budou obsahovat jednotlivé příspěvky. Pod-témata může přidávat také uživatel.
...
- **Způsob zobrazení textu stránky**
  ...
  - Normální - při zobrazení kompletního výpisu diskuse, se zobrazí i kompletní znění původní stránky
...
!>**Upozornění:** pokud chcete aby **každý** autor stránky s diskusí dostával notifikaci, můžete to nastavit konfigurační proměnnou ```forumAlwaysNotifyPageAuthor```, kterou nastavíte na hodnotu ```true```. Pokud ji nastavíte na hodnotu ```false```, bude to záležet na nastavení jednotlivých diskusí. **Totéž platí i pro Nástěnku diskusí**
...
- **Zapnout čas. limit pro mazání příspěvků** - zapněte jej, chcete-li umožnit mazání příspěvků jen dokud neuplyne časový limit. Na vymazání příspěvku má právo pouze jeho autor.
...
Karta **Skupiny** se zobrazuje pouze v případě zvolení komponenty **viactémová diskuse (Message Board)**. Nabízí možnost zadefinovat strukturu vícetémové diskuse pomocí skupin a podskupin.
