...
V seznamu formulářů se pro [víceleté formuláře](../multistep-form/README.md) zobrazuje také sloupec Trvání vyplnění, který zobrazuje jak dlouho trvalo vyplnění formuláře uživatelem (čas od jeho zobrazení po odeslání).
...
- **Umožni pouze jedno odeslání** - pokud je přihlášený uživatel a toto pole je nastaveno na hodnotu ```true```, tak pokud již daný uživatel formulář odeslal, systém mu nedovolí další odeslání. Formulář se takto bude v databázi od jednoho uživatele nacházet jen jednou.
- **Přepiš starší odeslání** - pokud je přihlášený uživatel a toto pole je nastaveno na hodnotu ```true```, tak pokud již daný uživatel formulář odeslal, bude jeho hodnota přepsána novou verzí. Formulář se takto bude v databázi od jednoho uživatele nacházet jen jednou.
...
- **Hlavička emailu** - pokud má vygenerovaný email obsahovat speciální hlavičku je možné do tohoto pole zadat čárkou oddělený seznam názvů polí, jejichž hodnoty se nastaví do hlavičky.

!>**Upozornění:** formulář očekává následně pojmenovaná pole pro správné nastavení emailu odesílatele a jeho jména:
...
3. Vytvořit stránku s textem e-mailu např. "Pro potvrzení platnosti email adresy klikněte na následující odkaz" a vložit odkaz na stránku, kde bude potvrzení souhlasu. Odkaz musí obsahovat parametry `!FORM_ID!,!OPTIN_HASH!`, tzn. `/potvrdenie-double-optin/?formId=!FORM_ID!&hash=!OPTIN_HASH!`. ID této stránky nastavit do pole ```Doc ID``` oznámení pro uživatele.
...
Pokud potřebujete implementovat podobnou funkčnost, ale s jinou logikou, pak v části Programování zákaznických aplikací je více technických informací o možnosti implementace [vlastního zpracovatele formuláře](../../../custom-apps/apps/multistep-forms/README.md), který zajistí požadovanou funkcionalitu.

## Událost při odeslání formuláře

Po odeslání formuláře přes AJAX je publikována událost `WJ.formSubmit`, na kterou lze poslouchat. jako:
...
- ```spamProtectionJavascript``` - ​​pokud je nastaveno na ```all``` (budou chráněny všechny formuláře) nebo ```formmail``` (chráněny budou jen formuláře odesílány na email), budou formuláře chráněny ```javascriptom```, pro deaktivování funkce je třeba zadat ```none```. Standardně nastaveno na ```all```.
...
- ```formMailSendPlainText``` - ​​pokud je nastaveno na ```true``` je email z formuláře odeslán jako čistý text (ne HTML formát).
- ```formMailRenderRadioCheckboxText``` - ​​pokud je nastaveno na ```true``` bude zobrazovat výběrová a zaškrtávací pole jako text ```[X]``` nebo ```[ ]```.
- ```formMailCropForm``` - ​​pokud je nastaveno na ```true``` bude se odesílat pouze část obalená do ```tagov``` form.
...
- ```formMailFixedSenderEmail``` - ​​Je-li nastaveno na email adresu, použije se jako fixní hodnota emailu odesílatele. Na rozdíl od ```emailProtectionSenderEmail``` nenastaví původní email do pole ```reply-to```, takže odesílatel není žádný oznámen při chybném doručení (což kvůli bezpečnosti může být někdy zapotřebí).
- ```formmailShowClassicErrorMessage``` - ​​Pokud je nastaveno na ```true``` bude zobrazovat klasické validační hlášení formuláře nahoře nad formulářem namísto hlášení u jednotlivých polí.
...
- ```formmailResetFormAfterSend``` - ​​Pokud je nastaveno na ```true``` po úspěšném odeslání se vyčistí formulář.
