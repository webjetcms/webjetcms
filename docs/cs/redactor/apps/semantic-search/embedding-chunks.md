...
Sémantický index převádí obsah stránek na vektorové reprezentace (`embedding`) pomocí OpenAI API a ukládá je do vektorové databáze. To umožňuje efektivní sémantické vyhledávání.
...
Indexuje se čistý text stránky – název, perex a tělo bez HTML tagů. Obsah se rozdělí na chunky, které jsou zobrazeny v tabulce níže.

Každý chunk obsahuje tyto sloupce:

- **ID entity** – ID webové stránky
- **Index části** – pořadí `chunk` v rámci stránky (0, 1, 2, ...)
- **Text části** – text, pro který byl vygenerován embedding (samotný embedding se nezobrazuje)
...
- **Datum vytvoření** – čas zpracování (ne přidání do fronty)
...
- **smazání nebo přesunutí do koše** – všechny související bloky se odstraní z databáze
...
Dialog zobrazí přehled stránek zvolené složky – celkový počet, počet již indexovaných a počet ve frontě. Složka se nastaví podle aktivního filtru. Po potvrzení se všechny stránky zařadí do fronty. Pokud stránka již má aktuální embedding, znovu se neindexuje.
...
Dialog zobrazí stejný přehled jako při indexování. Po potvrzení se všechny stránky zařadí do fronty pro odstranění všechny chunky pro stránky zvolené složky.
