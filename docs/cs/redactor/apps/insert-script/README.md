...
Aplikace „Skripty“ umožňuje vytváření skriptů, které můžete následně přidávat do libovolných stránek nebo skupiny stránek. Skripty lze vkládat podle souhlasu [návštěvníka s cookies/GDPR klasifikací](../gdpr/cookiesmanger.md).
...
- **Pořadí vkládání** – číselná hodnota určující pořadí vkládání skriptů v rámci stejné pozice. Skript s nižší hodnotou se vloží dříve. Výchozí hodnota pro stávající skripty je `10`. Při vytvoření nového skriptu, nebo zadání prázdné hodnoty, se automaticky nastaví na nejvyšší existující hodnotu v dané pozici + 10 (např. pokud nejvyšší existující priorita je `20`, nový skript dostane hodnotu `30`). Hodnotu lze kdykoli upravit.
...
- **Začátek platnosti** – datum a čas, od kdy je skript platný.
- **Konec platnosti** – datum a čas, do kdy je skript platný.
- **Vložit skript v editoru stránek** - určuje, zda se má skript vkládat v editoru stránek v režimu PageBuilder. Většinu skriptů není třeba v režimu editace vkládat, aby nekolidovaly s editorem stránky.
...
kde výraz `head` definuje pole **Umístění skriptu v šabloně**. Takže na uvedené místo se vloží všechny skripty, které mají nastavené pole Umístění skriptu v šabloně na hodnotu `head`. Skripty se vkládají v pořadí podle hodnoty pole **Pořadí vkládání** (vzestupně), při stejné prioritě se seřadí podle ID záznamu.