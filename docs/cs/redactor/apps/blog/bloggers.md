...
Chcete-li aby měl administrátor blogerů také možnost zasahovat do struktury a článků jednotlivých blogerů ([Seznam článků](./README.md)) musí mít i právo Blog/Seznam článků.
...
- Kořenová složka - rodičovská (nebo `root`) složka, kam se přidají blogerové složky. `/Blog`. Pro nově vytvořeného uživatele následně vznikne složka `/Blog/LOGIN`.
...
- Jelikož se jedná o uživatele typu **bloger**, je přidán do Skupiny uživatelů **Blog**.
...
- Bonusová práva je možné přidat, pokud před vytvořením blogeru upravíte konstantu ``bloggerAppPermissions`` o právu, které chcete aby mu byly po vytvoření povoleny.
...
Složka, kde se tato struktura vytvoří, byla dána při vytváření blogera parametrem Kořenová složka. V této složce se vytvoří pod-složka s názvem odpovídající hodnotě Přihlašovací jméno daného blogera. Protože každý uživatel musí mít unikátní přihlašovací jméno (`login`), nemůže nastat situace s kolizí jmen složek. Pro každou takovou vytvořenou složku se vytvoří ještě jedna podsložka/sekce s názvem **Nezařazené**.

Příklad:

Pokud jako **Kořenová složka** byla zvolena `/Aplikácie/Blog/` tak po vytvoření blogera s přihlašovacím jménem `bloggerPerm`, vznikne taková struktura v sekci web stránky:
...
Parametr `isBlog=true` způsobí vypnutí diskuze pro hlavní stránky složky (kde se typicky nachází seznam článků ve složce).
