...
Asistent u textového pole (jednořádkové pole i `textarea`) slouží k vygenerování nového obsahu nebo úpravě existujícího. Spouští se přes tlačítko <button class="btn btn-outline-secondary btn-ai" type="button"><i class="ti ti-sparkles"></i></button> na konci pole.
...
Jelikož i pole typu `Quill` patří mezi textová pole, AI asistenti pro něj fungují stejně jako pro klasická textová pole. Jediný rozdíl je v umístění ikony, která není na konci pole, ale v hlavičce mezi nástroji.
...
Asistenti fungují také v `PageBuilder` a slouží k vygenerování nového obsahu nebo úpravě existujícího obsahu stránky vloženého v `PageBuilder`. Spouštějí se přes ikonu<button class="btn-ai" type="button"><i class="ti ti-sparkles"></i></button> umístěnou v panelu nástrojů.
...
Provedené úpravy se zobrazí ve webové stránce a znovu můžete kliknutím na Pokračovat zadat další požadavek, nebo kliknutím na **OK** asistenta ukončit. Všimněte si na fotce, že podle zadaného požadavku je upraven nadpis oproti předchozí verzi.
