...
-<button class="btn btn-sm btn-danger"><span><i class="ti ti-cut"></i></span></button> , **Vystřihnout**, zkopíruje označený soubor/složku ze zdrojové lokace. Po vložení do cílové lokace se originál ze zdrojové lokace vymaže.
...
- <button class="btn btn-sm btn-outline-secondary"><span><i class="ti ti-layout-grid"></i></span></button> , **Ikony**, změní zobrazení souborů/složek v pracovní zóně pro zobrazení pomocí ikon.
...
- <button class="btn btn-sm btn-outline-secondary"><span><i class="ti ti-arrows-up-down"></i></span></button> , **Seřadit**, umožňuje seřadit soubory/složky podle vybraného kritéria.
...
- <button class="btn btn-sm btn-outline-secondary"><span><i class="ti ti-file-download"></i></span></button> , **Stáhnout**, umožňuje stažení vybraného souboru (POUZE souboru).
...
-<button class="btn btn-sm btn-outline-secondary"><span><i class="ti ti-eye"></i></span></button> , **Náhled**, poskytuje rozdílnou funkcionalitu v závislosti na zvoleném elementu, základem je vždy dialogové okno, které lze zvětšit/zmenšit. Najednou může být otevřeno vždy **jen jedno okno**. Při výběru jiného souboru/složky se okno aktualizuje (neotevře se nové). Pomocí šipek umíte měnit zvolený soubor/složku v aktuálně zvolené lokaci.
  ...
  - Textový soubor jako `text/plain`, `text/html`, `text/jsp`, `text/javascript`, `text/css`, `text/xml`, `text/x-js`, `text/markdown`, otevře soubor v dialogovém okně (soubor se však **nedá upravit**).
...
!>**Upozornění:** velikost složky počítá velikost souborů v dané složce, nepočítá soubory v podsložkách.
...
Šířka navigační lišty není fixní, a lze ji měnit podle potřeby. Stromová struktura podporuje také `Drag and Drop`, čili umožňuje přesouvání složek. Každý řádek se složkou může obsahovat ikony:
...
Na hlavní ploše máme všechny složky a soubory aktuálně zvolené složky.
!>**Upozornění:** nezobrazují se vnořené složky a soubory.

Jako "Navigační lišta", "Hlavní plocha" také podporuje `Drag and Drop` akci.
!>**Upozornění:** akce `Drag and Drop` funguje také mezi "Hlavní plocha" a "Navigační lišta", takže můžete složku/soubor přesunout z plochy přímo do složky navigační lišty.

**Levý klik**, slouží k označení složek a souborů.

**Dvojitý levý klik**, na:

- složka, provede její otevření
- na textový soubor typů `text/plain`, `text/html`, `text/jsp`, `text/javascript`, `text/css`, `text/xml`, `text/x-js`, `text/markdown`, spustí akci úpravy
- na jiné nepodporované soubory jako `.tld`, neudělá nic

**Pravý klik** slouží hlavně k zobrazení nejpoužívanějších nástrojů z [Panelu nástrojů](#panel-nástrojů). Zobrazí se malé okno s dostupnými nástroji, které se mohou lišit podle toho, na jak elementu se provedli pravý klik.
...
Jak můžete vidět z předchozí série obrázků, zobrazené nástroje se liší v závislosti na zvoleném elementu. Například pro soubor máme zobrazenou možnost **Stáhnout** ale pro složku ne, nakolik povoleno je POUZE stahování souborů.
Také jste si mohli všimnout nástrojů, které jsme si v sekci [Panelu nástrojů](#panel-nástrojů) nezmínili, protože nemají vlastní tlačítko v panelu. Jedná se o nástroje:
...
-<span><i class="ti ti-file-upload"></i></span> , **Aktualizovat soubor**, je to akce nad souborem, umožňuje nahrát nový soubor, který nahradí zvolený soubor (nad kterém jsme volali tuto akci).
